...
|---|---|---|
| **Menu** (Main Menu / Submenu) | menu screen | GOLD screen generated by `run_menu()`/`print_header()` — a header with a centered title, a numbered list of options, and a fixed last item (`Back`/`Exit`), set apart by a blank line and always selected with `X` (never a number). See "The Four Screen Helpers". |
...
graph TD
    ...
    N -->|Search by state| P
    P -->|Return| N
    N -->|Toggle a flag on a change| TF["🏷️ Toggle flag<br/>Listing by id (loop) + Inline Selection (loop)<br/>+ set-metadata.py (no confirmation)"]
    TF -->|Return| N
...
Second case, for a different reason: `install_previo_version()`'s "Allowed Previo versions:" listing **is** row-numbered, but still bypasses `show_selection()` — that helper returns `None` for both "empty input" and "typed something invalid," with no way for the caller to tell them apart, while this screen needs the two to behave differently ("go back" vs. "Invalid option., stay on the same screen"). So it renders the same `hr("-")` + numbered-list + `hr("-")` frame `show_selection()` would, but reads the choice with `read_input()` and branches on empty vs. out-of-range itself.

**Older-than-installed tags are filtered out, not offered with a warning.** `install-framework.py` hard-rejects installing a tag older than the currently installed version unconditionally — even with `--yes`, with no override (see "External Dependencies") — so `install_previo_version()` never lists one in the first place: listing it and letting the user confirm would just guarantee a later failure. Before building `options`/`tags`, both `official_tag` and `prerelease_tag` are checked against `framework_version()` (the installed version) via `_semver_tuple()` — a small `(major, minor, patch)` parser duplicated from `install-framework.py`'s own `parse_version()`, kept in sync by convention, same self-contained-script pattern the rest of this file follows — and any tag strictly older than installed is dropped. This is the case where a pre-release is installed and the latest "official" release predates it: only the pre-release tag (equal to what's installed) survives the filter, not the older official one. **If every known tag gets filtered out**, the list would be empty — instead of showing an empty "Allowed Previo versions:" listing, `install_previo_version()` shows an Info (`framed=False`) saying Previo is already up to date and returns straight to "Previo: settings", never entering the loop. **The one remaining, non-blocking case:** picking the **exact installed tag** (still allowed, since it isn't older, just equal) prints a plain (uncolored) warning line right before the `confirm()` prompt — continuing the same DARK_GRAY-free block, no `hr()` of its own, same as any text right before a Confirmation — that this will reinstall everything from scratch. It doesn't block the pick or change `confirm()`'s wording, it's just informational context read right before answering the same `"Install Previo {tag}? This modifies files in this project."` question every other pick gets.
...
Everything in GOLD: the header (top, title, bottom) and also the `hr("=", GOLD)` that closes the option list before the "Choose an option:" prompt. `hr()`'s default is DARK_GRAY, so any new call to `hr()` inside `run_menu()` needs an explicit `color=GOLD` (see "Common Mistakes When Extending", point 1). The last item (`Back`/`Exit`) is always printed after a blank line, set apart from the numbered options above it, and its key is always the letter `X` — never a number, and never renumbered when options are added or removed.
...
1. Follow the pattern from `close_entry()` / `toggle_flag_on_change()`: pick the target (`show_selection()` for a short, numberable list; an own listing + `read_input()` of an id when it's grouped and the code is the natural handle, as in `toggle_flag_on_change()`), followed by `confirm()` **if the action isn't trivially reversible**. "Close entry" moves a folder (destructive from the user's view) → `confirm()`. "Toggle a flag" just flips a boolean in `.metadata.json` and the same action undoes it → **no `confirm()`**, applied immediately with the updated list re-shown. The rule: `confirm()` for what's costly to undo, not for everything that writes.
...
4. If the target listing is long or heterogeneous, group it with headings like `toggle_flag_on_change()` does — reusing the same partition as "General project status" (🟢 `implemented/*`, 🟠 `inProgress/*` with `plan.md`, 🟡 `inProgress/*` without `plan.md`; `closed/*` is left out as frozen). `show_selection()` can't render group headings: print the listing yourself and, since each entry's code is already a stable handle, show the rows **unnumbered** (`{code} — {name}`) and read the choice with `read_input()`, resolving the typed id against the entries with an `_ids_match()`-style matcher (zero-padding ignored) — not a parallel row numbering. The group headings are tinted **GOLD** (a scoped exception to "one color per screen", same as `search_by_state()`'s per-state colors) to set them apart from the DARK_GRAY entry rows.
