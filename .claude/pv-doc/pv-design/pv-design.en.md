...
- **pv-update** — Audits and repairs `.claude/pv-context.json` and everything it configures against the real state of the repo (schema shape, obsolete keys, referenced skills, on-disk paths, the `architectureDocDir` namespace seed, `stuff/hooks/*/*.md` seeds, `pv.py` freshness, `skillModels` drift, the `[[[...]]]`-marked structural labels, duplicate change codes, and version consistency across every `pv-*` skill) — fixes everything it can determine unambiguously and reports what changed; only asks the user when `pv-context.json` is broken JSON, the installed version looks like a downgrade, or a namespace anchor's file no longer exists. It's also the framework's own install/update mechanism (`/pv-update install`): resolves a target version (latest official release, or an explicit `--version`), rejects a downgrade or an unparseable tag with no escape flag, and — only if the check passes — delegates the actual download/sync to `install.sh`/`install.ps1` at the repo root. This second mode never runs on its own; the default, audit-only flow never touches the network or installs anything. Every other public `pv-*` skill's step 0 runs `check-framework-status.py` (below) as a cheap gate before doing any of its own work, and sends the user here when it fails. *Uses:* no other skill.
  ...
  - [`workflow.install.md`](skills/pv-update/workflow.install.md) — Mermaid diagram of the install mode's flow (see "Workflow diagrams" above): query GitHub Releases for the latest official version and any newer pre-release, resolve the target version, reject a downgrade or unparseable tag, warn (without blocking) if the target is a pre-release, invoke `install-framework.py`, report and remind the user to run `/pv-update` (audit mode) next.
  - [`scripts/install-framework.py`](skills/pv-update/scripts/install-framework.py) — detects the OS (`platform.system()`, never by which binaries are on `PATH`) and invokes `install.ps1` (Windows) or `install.sh` (else) at the repo root with the resolved tag. Queries `GET /repos/{REPO}/releases/latest` and `GET /repos/{REPO}/releases?per_page=1` to always report the latest official release and, if newer, a pre-release, before deciding anything. Hard-rejects, with no escape flag, an unparseable `--version` tag or a real downgrade against the installed version — never delegates that comparison to `install.sh`/`.ps1`. Never runs `mark-verified.py` itself.
