...
- **pv-init** — Initializes the framework: creates/completes `.claude/pv-context.json` (`framework.workFolder` — fixed at `/previo-sdd`, never asked, a repo-relative root under which the framework manages `changes/`, `versions/`, and `stuff/`, fixed-name subfolders that the skills create themselves —, docs to sync, language configuration) and checks that the required command-line tools are installed. Before touching any configuration, its step 0 verifies the dev tooling the framework itself needs (Git, Python 3) plus whatever the detected project type adds (e.g. Node/npm for a `package.json`), asking before installing anything and letting the user proceed anyway or stop if something can't be installed. On a first `pv-init`, it always asks for the interaction language (`framework.interaction.language`) and, with a yes/no, whether the other areas (`changes`, `versions`, `docs.functional`) share that same language or are configured one by one; it records the rationale for each choice in `framework._comments`. `docs.tech` isn't part of that question — it's always technical English. If the project was already initialized without a configured language (`hasLanguage: false`), it adds that question to the same round that completes the rest of the pending optionals, without asking again if it was already resolved. The three documentation paths (`docs.functional.featuresDocPathDir`, `docs.tech.architectureDocDir`, `docs.tech.styleBibleDocDir`) are not among the "optional" fields it can leave unset — they're always written and scaffolded; it never asks whether the project wants that documentation. If `sourcecodeDir` already holds real code (not just an empty scaffold) at init time, it asks the user to choose a documentation depth — **minimal** (faster, lower token cost, improves over time as changes/fixes land) or **full** (slower, higher token cost, complete from the start) — then, once the rest of the configuration is written, invokes `pv-internal-tech-analysis` (with `bootstrap: true`, so it explores the real code instead of resolving the still-placeholder `docs.tech`) to turn the three scaffolded placeholders into real initial documentation at that depth: architecture content plus a file/symbol map, a style bible (only if the project has a presentation layer — a GUI or a CLI counts), the exhaustive feature list, and the `00-namespace.md` seed populated. Skipped entirely if there's no existing code to analyze. The single configuration point that all the other skills depend on. *Uses:* no other skill.
  ...
  - [`scripts/scaffold-project.py`](skills/pv-init/scripts/scaffold-project.py) — creates the base folder structure (`changes/{inProgress,implemented,todo,closed}`, `versions/`, `stuff/`, `stuff/hooks/`) and any missing placeholders for `docs.tech.*`/`docs.functional.featuresDocPathDir`, and overwrites `pv.py` in the repo root with the copy from `assets/pv.py`. It also creates one `stuff/hooks/<subdir>/` per hook-exposing skill (see its `HOOK_SETS`) and copies that skill's hook files into it as `NN-slug.md` **only if each doesn't exist** — `stuff/hooks/version/` gets `pv-version`'s four ([`hooks/05-before-guardrail.template.md`](skills/pv-version/hooks/05-before-guardrail.template.md), `10-before-version`, `20-after-build`, `30-after-changelog`), `stuff/hooks/do/` gets `pv-do`'s two ([`hooks/10-before-implementation.template.md`](skills/pv-do/hooks/10-before-implementation.template.md), `20-after-implementation`), `stuff/hooks/how/` gets `pv-how`'s two ([`hooks/10-before-analysis.template.md`](skills/pv-how/hooks/10-before-analysis.template.md), `20-after-plan`), `stuff/hooks/new/` gets `pv-new`'s one ([`hooks/20-after-entry.template.md`](skills/pv-new/hooks/20-after-entry.template.md)). Never overwrites one, so a project that already added steps keeps them; same idea as `docs/*` starting with its `INDEX.md`/`001-overview.md` rather than empty. Unlike `NAMESPACE_SEED` (inlined in the script), the hook seeds are read from the `.template.md` files at runtime (like `assets/pv.py`).
...
- **pv-fix** — Documents a bug and implements it end to end, and is also the framework's fast path for changes so small they barely require analysis (a typo, a text string, a value/constant, an isolated style tweak, whether or not it's a bug). It first invokes `pv-internal-tech-analysis` to assess whether the request is `fast` (unambiguous, ≤2 files, no impact on `docs.tech.architectureDocDir`/`docs.tech.styleBibleDocDir` and no detected inconsistencies with them, no new behavior). If it's `fast`, it creates the entry via `pv-internal-workflow` (`action=create`, `type=fast`), applies the change directly, and moves it to `implemented` (`action=move`) in the same invocation, without `plan.md`. Because the fast-track edits code the same way `pv-do` does, it runs `pv-do`'s two project hooks (`{workFolder}/stuff/hooks/do/10-before-implementation.md` before touching code, `20-after-implementation.md` after applying and documenting the change, before the folder moves) — the same files, matched the same way; there is no separate `fix/` hook set. If it's not `fast` and it's a bug, it generates `description.md` via `pv-internal-workflow` (`type=fix`), invoking `pv-internal-tech-mermaid`/`pv-internal-mockups-html` when the fix has a flow or visual component to represent, and automatically chains `pv-how` to fix it end to end, with the analysis strictly confined to the root cause (no scope expansion). If it's not `fast` and it's not a bug, it warns the user and invokes `pv-new` with their request. *Uses:* `pv-internal-workflow`, `pv-internal-tech-analysis`, `pv-internal-tech-mermaid`, `pv-internal-mockups-html`, `pv-new`, `pv-how`.

  Assets and scripts:
  - [`workflow.fix.md`](skills/pv-fix/workflow.fix.md) — Mermaid diagram of this skill's full flow (see "Workflow diagrams" above), covering both the fast-track subflow and the non-trivial one; read before executing any step, the source of truth for the sequence and the branches. The fast-track subflow draws `pv-do`'s two hook points (`10-before-implementation`, `20-after-implementation`) in orange — it reuses `pv-do`'s hook set, it doesn't define its own.
  - Reuses `extend-entry.md` from `pv-new` when the given `xxxx` already exists in `inProgress`.

- **pv-how** — Takes an entry already documented in `inProgress`, invokes `pv-internal-tech-analysis` to gather the technical context, analyzes the technical solution, and writes `plan.md` (using `pv-internal-tech-mermaid`/`pv-internal-mockups-html` when what needs describing is a flow or requires a visual mockup). With `plan.md` written, it invokes `pv-internal-tech-risks` to assess the risk of breaking something when implementing it, and writes the returned median into `.metadata.json` (the `risk` field) via `set-metadata.py --set-risk` — not into `plan.md` (the detail of the 9 factors is only added if the user asks for it, in the plan's `(f)` section). If the user confirms they want to implement now, it chains directly into `pv-do` on the same entry. *Uses:* `pv-internal-tech-analysis`, `pv-internal-tech-mermaid`, `pv-internal-mockups-html`, `pv-internal-tech-risks`, `pv-do`.
...
- **pv-status** — Gives a read-only overview of the project state (totals by type — including `fast`, the trivial shortcut of `pv-fix` — and by state, a breakdown of what is only described vs. ready to implement, and a separate listing of the `fast` changes already applied). It reads each change's **flags** from its `.metadata.json` and shows them as icons in every listing; it also offers `filter_status.py --flag <name>` to list the changes carrying a given flag, and `read-flags.py` as a read-only service for `pv.py`. It creates, moves, or modifies nothing; the report is delivered in chat unless the user asks to save it. *Uses:* no other skill.
  ...
  - [`scripts/collect_status.py`](skills/pv-status/scripts/collect_status.py) — walks `{changesDir}` and returns a JSON with the detail and aggregated totals of each entry (type, name, whether it has `description.md`/`plan.md`, sub-state within `inProgress`, **`risk`** and **`flags`** read from `.metadata.json`); it writes nothing, it only diagnoses. Exposes `read_flags()` / `read_risk()` / `read_metadata()` reused by the other scripts.
...
- **pv-review-doc-tech** — Reorganizes, structure only, every folder configured under `framework.docs.tech` (today `architectureDocDir` and `styleBibleDocDir`; it reads the keys present there dynamically instead of hardcoding the two, so a future third folder is picked up without a skill change). It never deletes a fact, rewrites a sentence, or adds content — only moves it to the file/`**Area**`/notation shape it already should have under the same rules `pv-do` applies when it creates that documentation: one file per topic (`pv-internal-doc-files`' `find`/`upsert` convention), the category checklists of `pv-internal-doc-technical`/`pv-internal-doc-style` to judge where content belongs, and `pv-internal-doc-technical`'s notation-first writing rules to judge internal disorder within a file. It reads an entire folder (`INDEX.md`, every `{NNN}-{slug}.md`, and `00-namespace.md`) before deciding anything, so a duplicate spread across two files or a namespace-path collision is caught — a per-file pass alone would miss both. It never renumbers/renames a `{NNN}-{slug}.md` file, never hand-edits `INDEX.md` (only `rebuild-index.py` does), and never edits `00-namespace.md` itself — a namespace change it identifies as needed is flagged to the user instead of applied. It doesn't invoke `pv-internal-doc-technical`/`pv-internal-doc-style`/`pv-internal-doc-files` as live Skill-tool calls (there's no content to draft or file to place that would need them mid-flow); it applies their already-documented rules directly while reorganizing, and only invokes `pv-internal-doc-files`' `rebuild-index.py` script once edits to a folder are done. *Uses:* no other skill (applies `pv-internal-doc-technical`/`pv-internal-doc-style`/`pv-internal-doc-files`'s rules directly rather than invoking them).
...
- **pv-update** — Audits and repairs `.claude/pv-context.json` and everything it configures against the real state of the repo (schema shape, obsolete keys, referenced skills, on-disk paths, the `architectureDocDir` namespace seed, `stuff/hooks/*/*.md` seeds, `pv.py` freshness, `skillModels` drift, the `[[[...]]]`-marked structural labels, duplicate change codes, and version consistency across every `pv-*` skill) — fixes everything it can determine unambiguously and reports what changed; only asks the user when `pv-context.json` is broken JSON, the installed version looks like a downgrade, or a namespace anchor's file no longer exists. It's also the framework's own install/update mechanism (`/pv-update install`): resolves a target version (latest official release, or an explicit `--version`), hard-rejects a downgrade or an unparseable tag with no escape flag, warns (without blocking) when the resolved tag exactly matches the installed version (a reinstall from scratch, not an upgrade), and only installs after the user has explicitly confirmed that exact resolved tag by name — enforced by a two-step protocol (`install-framework.py` resolves and validates first, installs only when re-run with `--yes`) so this skill can never install a version the user hasn't confirmed. `pv.py`'s own "Install new Previo version" menu option applies the same downgrade rule and the same same-version warning, reached from a different entry point (see [`pv-design-onescript.en.md`](../pv-doc/pv-design-onescript/pv-design-onescript.en.md)) — `pv.py` filters an older-than-installed tag out of its list entirely instead of offering it and failing later, since it only ever lists two known tags (official + pre-release) rather than accepting free-form user input. This second mode never runs on its own; the default, audit-only flow never touches the network or installs anything. Every other public `pv-*` skill's step 0 runs `check-framework-status.py` (below) as a cheap gate before doing any of its own work, and sends the user here when it fails. *Uses:* no other skill.

  Assets and scripts:
  - [`workflow.audit.md`](skills/pv-update/workflow.audit.md) — Mermaid diagram of the audit-only flow (see "Workflow diagrams" above): read `pv-context.json` best-effort, run `audit-context.py`, walk each reported problem applying its deterministic fix (or asking, for the four exceptions that need judgment), write the verified version, report. Source of truth for this mode's sequence and branches.
  - [`scripts/audit-context.py`](skills/pv-update/scripts/audit-context.py) — the full, read-only auditor: every deterministic check this skill runs, printing a single JSON of `problems`. Never writes anything; slow enough (walks the whole `changes/` tree, templates, hooks) that it only runs on demand, not on every other skill's invocation.
  - [`scripts/mark-verified.py`](skills/pv-update/scripts/mark-verified.py) — writes `framework.frameworkStatus` after an audit run: `--clear` (healthy, or an outdated case just resolved), `--block "<reason>"` (a downgrade was detected), `--confirm-downgrade` (the user confirmed it was intentional).
  - [`scripts/check-framework-status.py`](skills/pv-update/scripts/check-framework-status.py) — the cheap subset every other public `pv-*` skill runs on every invocation, not just `pv-update` itself: no arguments, prints one JSON (`ok`, `problem`, `message`), stopping at the first of `context-missing` → `context-invalid-json` → `blocked` → `version-mismatch` → `skill-count-mismatch` (the exact number of installed `pv-*` skill folders, `EXPECTED_SKILL_COUNT`, kept in sync by `tools/set-skill-versions.py` on every release cut). Deliberately a fast binary gate, not the full auditor — never duplicates `audit-context.py`'s expensive checks.
  - [`workflow.install.md`](skills/pv-update/workflow.install.md) — Mermaid diagram of the install mode's flow (see "Workflow diagrams" above): query GitHub Releases for the latest official version and any newer pre-release, resolve the target version, reject a downgrade or unparseable tag, warn (without blocking) if the resolved tag exactly matches the installed one or if it's a pre-release, ask the user to confirm that exact resolved tag by name, only then re-invoke `install-framework.py` with `--yes`, report and remind the user to run `/pv-update` (audit mode) next.
  - [`scripts/install-framework.py`](skills/pv-update/scripts/install-framework.py) — resolves a single target tag (explicit `--version`, or the latest official release) and, without `--yes`, only validates it and stops: queries `GET /repos/{REPO}/releases/latest` and `GET /repos/{REPO}/releases?per_page=1` to always report the latest official release and, if newer, a pre-release; hard-rejects, with no escape flag, an unparseable tag or a real downgrade against the installed version, for that resolved tag regardless of whether it came from `--version` or from "latest" (an unattended "install latest" that would silently downgrade an installed pre-release is rejected the same as an explicit older `--version`). Only with `--version <tag> --yes` does it actually install: detects the OS (`platform.system()`, never by which binaries are on `PATH`), deletes any `install.ps1`/`install.sh` it finds at the repo root (never trusted, even if present) and downloads a fresh copy from previo-sdd's `main` branch into a temp file, runs it with the resolved tag, and deletes the temp file again afterward — regardless of success or failure. Never runs `mark-verified.py` itself. What that downloaded script actually does (version/tag resolution, download with a fake-progress bar, selective sync) is documented on its own: [`pv-install-scripts.md`](../pv-install-scripts/pv-install-scripts.md).

- **pv-version** — Prepares a release in `{workFolder}/versions/{XXXX}/`: it first requires `{changesDir}/implemented/` to be empty (each entry is resolved by moving it to `closed`), generates the deliverable following `{workFolder}/stuff/how-to-compile.md` (a project-specific procedure, written the first time it's needed, able to describe several steps if the build generates several artifacts), compresses into `.zip` and copies whichever of `docs.tech.architectureDocDir`/`docs.tech.styleBibleDocDir`/`docs.functional.featuresDocPathDir` are configured, and chains `pv-internal-changelog` for the functional changelog. If invoked only to report a change in the build procedure, it updates `{workFolder}/stuff/how-to-compile.md` without running the rest of the process unless explicitly confirmed. `{XXXX}` is free text chosen by the user on each invocation, with no relation to the `xxxx` numbering of a change/fix nor to any other "versions" folder that exists in the repo. *Uses:* `pv-internal-changelog`.

  **Customization points (the skill isn't edited).** The `SKILL.md`, `workflow.version.md`, and other files under `.claude/skills/pv-*/` are installed framework, managed via `pv-update`; editing them by hand leaves them inconsistent. There are exactly two customization points in `{workFolder}/stuff/`: how to compile the deliverable → `how-to-compile.md` (steps 3–4); the project's own steps at four points of the flow → the hook files under `hooks/version/` (step 0.6), one per point: `05-before-guardrail.md` (step 0.4, before the `implemented/` guardrail and before step 1, only `{workFolder}` substituted; `{XXXX}` not resolved yet), `10-before-version.md` (step 0.7, after the guardrail, before step 1, same variables), `20-after-build.md` (step 4.1, after the artifacts are copied to `files/`; `{XXXX}` and the `versions/{XXXX}/` paths available) and `30-after-changelog.md` (step 6.1, after the changelog and before the summary; same variables). Each holds 0..N `### Step N: {name}` blocks in the same shape as `how-to-compile.md`; `pv-version` matches a file by its `NN-` id prefix (the `slug` is fixed too). A file with no steps, or absent, is skipped silently; if a step fails, the flow stops and explains it (same criterion as step 4). The final summary (step 7) reports which hooks ran and what they produced. If what the user asks doesn't fit either point, they're told so and pointed at opening a change in the framework repo — never a local patch to the skill.

  Assets and scripts:
  - [`workflow.version.md`](skills/pv-version/workflow.version.md) — Mermaid diagram of this skill's full flow (see "Workflow diagrams" above); read before executing any step, the source of truth for the sequence and the branches. It includes the four hook points (`05-before-guardrail` before the `implemented/` guardrail, `10-before-version` after the guardrail and before resolving `XXXX`, `20-after-build` after copying artifacts, `30-after-changelog` after the changelog). Not to be confused with `version-flow-diagram.template.md` below: this one is the complete internal flow (every `ASK`/`INFO` node and branch), the other is a simplified user-oriented view.
  - [`how-to-compile.template.md`](skills/pv-version/how-to-compile.template.md) — the template that `pv-version` copies to `{workFolder}/stuff/how-to-compile.md` the first time it's needed: command(s) to run, generated file(s), and notes, with support for several independent steps if the build generates several artifacts.
  - [`hooks/*.template.md`](skills/pv-version/hooks/) — the seeds for `{workFolder}/stuff/hooks/version/<NN>-<slug>.md`, one per insertion point (`05-before-guardrail`, `10-before-version`, `20-after-build`, `30-after-changelog`): a header explaining when the hook runs and which variables are available, plus a commented-out `### Step N` skeleton, zero live steps. It's `pv-init`'s `scaffold-project.py` that copies them (not `pv-version`), each only if absent; `pv-version` (step 0.6) matches a file by its `<NN>` id prefix and runs whatever `### Step N` blocks it contains. Never overwritten once a file has content.
  ...
  - [`version-flow-diagram.template.md`](skills/pv-version/version-flow-diagram.template.md) — a general, simplified Mermaid diagram of the `pv-version` process (the project's own `05-before-guardrail` hook step → the empty-`implemented/` guardrail → the project's own `10`/`20`/`30` hook steps (`stuff/hooks/version/*.md`) → create folder → compile → compress docs → changelog → confirm), meant to be shown as-is if the user asks how `/pv-version` works — only the information relevant and understandable to the user, not all the internal branching that `workflow.version.md` does cover. The hook nodes are orange (`classDef extension`), distinct from the blue mechanical steps.
...
- **pv-internal-workflow** — Centralizes the framework's file mechanics: numbering and creating new entries in `inProgress` (`action=create`, with `type` `change`/`fix`/`fast`), and moving folders between states (`action=move`). It analyzes and decides nothing, it only executes what the calling skill already resolved. For the `fast` shortcut of `pv-fix`, the caller typically chains `create` and `move` in the same invocation, without going through `plan.md`. It is also the **owner of the `.metadata.json` contract** (the per-change mutable-state file: today the *flags*) and of `set-metadata.py`, the sole writer of that file. *Uses:* no other skill.
  ...
  - [`metadata.schema.json`](skills/pv-internal-workflow/metadata.schema.json) — JSON Schema for `.metadata.json` (`{workFolder}/changes/{state}/{xxxx}/.metadata.json`, an optional dotfile): `flags` (array, enum `priority`/`workinprogress` — the framework's **canonical flag catalogue**), `flagsLastModified` (ISO date of the last `flags` mutation), and `risk` (integer 0-10 | absent — the median of `pv-internal-tech-risks`' 9 factors, written by `pv-how` in step 3.1 via `set-metadata.py --set-risk`; absent = not yet assessed). Referenced by `pv-status` when it reads the file and by `pv-update` when it audits it.
  - [`scripts/set-metadata.py`](skills/pv-internal-workflow/scripts/set-metadata.py) — the sole writer of `.metadata.json`. `--xxxx <code>` (+ state resolution by searching for the folder, or explicit `--state`), `--add-flag`/`--remove-flag`/`--toggle-flag <name>` (repeatable, validated against the enum), `--set-risk N` (integer 0-10; independent of flags, doesn't touch `flagsLastModified`), `--work-folder`, `--print`. A single invocation can touch flags, `risk`, or both. Read-modify-write with a **file lock**; rejects any operation under `todo/`; creates the file if absent and **never deletes it** (even when `flags` ends up `[]`); preserves unknown fields.
...
- **pv-internal-mockups-html** — Generates or edits static visual mockups in self-contained HTML/CSS/SVG (`design_*.html`) of a new or modified UI element, from the destination folder and the list of elements the caller needs to mock up, reading the project's style bible (`docs.tech.styleBibleDocDir`, read-only) first so the mockup replicates the documented visual identity rather than an invented one. If the style bible doesn't cover an element, it doesn't fall straight back to a neutral placeholder: it looks for the real styling already in `sourcecodeDir` (theme/token files, stylesheets, the closest existing component — read-only, same as the style bible) and reuses that instead. Either way the gap is real, so it reports it back to the caller as a **style gaps found** list (what was reused from code, and where), so the caller can flag the missing documentation instead of it going unnoticed. It doesn't decide which elements are needed or validate anything with the user, it only produces the files and returns their paths plus that gap list. It's the default mockup skill for `framework.skills.mockups`. Used by `pv-new` and `pv-fix`. *Uses:* no other skill.

  Assets and scripts:
  - [`assets/mockup-annotations.html`](skills/pv-internal-mockups-html/assets/mockup-annotations.html) — the master copy of the **mnoteqz7k-framework**, a self-contained review-annotation runtime (floating toolbar, pin-to-element notes, general/linked notes panels, show/hide, save via the File System Access API with a Blob-download fallback). Its whole namespace (ids, classes, the marker comment) carries the `mnoteqz7k` prefix rather than the more obvious `mnote`, deliberately — a short, effectively-random tag makes an accidental collision with a real mockup's own naming implausible, which matters because the ID collision guard's whole job is telling "framework already embedded" apart from "unrelated element that happens to share the name". No demo body on purpose — only the `<!-- mnoteqz7k-framework vN -->` marker, `<style id="mnoteqz7k-styles">`, an empty `#mnoteqz7k-data`, and `<script id="mnoteqz7k-runtime">`, the exact blocks `pv-internal-mockups-html` is meant to splice verbatim into a `design_*.html` it generates (this splicing isn't implemented in `SKILL.md` yet — see `.claude/plans/interactive-mockups/PLAN.md`, still mid-implementation as of this writing).
  - [`assets/SANDBOX.md`](skills/pv-internal-mockups-html/assets/SANDBOX.md) — dev-only instructions for manually testing the mnoteqz7k-framework against a wide variety of interactive widgets and complex nested DOM (a separate builder script plus a golden/working sandbox pair, so tests never contaminate the master asset). `install.sh`/`install.ps1` strip every `*.sandbox.*` file and the builder script from what they copy into a consuming project — `SANDBOX.md` and the master asset are the only files from this folder meant to ship.

- **pv-internal-mockups-ascii** — The same function and the same input/output contract as `pv-internal-mockups-html` (including the same style-bible-then-code fallback and **style gaps found** reporting, applied to layout and microcopy instead of visual styling), but generating the mockups as plain-text ASCII art (`design_*.txt`) instead of HTML. It's only invoked when a project configures `framework.skills.mockups` to use this alternative instead of the default. *Uses:* no other skill.
...
**pv-review-doc-tech** (user-invocable, listed above) isn't in this diagram — it never invokes `pv-do` or any of these four skills as a live process. It's a second, separate consumer of the same two content-checklist skills (`pv-internal-doc-technical`/`pv-internal-doc-style`): where `pv-do` calls them to decide **what new content to write**, `pv-review-doc-tech` applies their categories and writing rules **as a fixed standard to reorganize existing content against**, without calling them mid-flow (there's nothing for them to return that changes a structural move already decided from reading the checklist). It does call `pv-internal-doc-files`' `rebuild-index.py` script directly, the same regeneration step `doc-files`' own `upsert` action triggers.
...
- **`workFolder`** (`string`, optional, default `"/previo-sdd"`): a repo-relative folder under which the framework manages all of its work. The leading `/` is just a visual convention (so it stands out at a glance from `docs.*`, which is relative to `workFolder` itself) — it's optional and every `pv-*` script strips it before resolving the path, so `"previo-sdd"` and `"/previo-sdd"` work the same; it's never a real absolute filesystem path. It's the only `framework` field that `pv-init` never asks about or confirms: it always writes the default silently, just like `skills.mockups`/`skills.diagrams`. If you ever want a different folder, change it by hand in `pv-context.json`, at the risk of whoever edits it. It's resolved via `resolve-path.py` — see "Path resolution" below. Inside it, `pv-init`'s `scaffold-project.py` creates three fixed-name subfolders right after writing `pv-context.json`, which the user doesn't choose or rename:
  ...
  - `{workFolder}/stuff/` — project-specific files that no other framework skill decides for it: `how-to-compile.md` (the build procedure that `pv-version` asks about and writes the first time it's needed) and `hooks/` (a subfolder per hook-exposing skill; `hooks/version/` holds `pv-version`'s four hook files, read in its step 0.6 — `05-before-guardrail` run in step 0.4; `hooks/do/` holds `pv-do`'s two — `10-before-implementation.md`, `20-after-implementation.md` — read in its step 1.5, and also by `pv-fix`'s fast-track branch (which edits code directly); `hooks/how/` holds `pv-how`'s two — `10-before-analysis.md`, `20-after-plan.md` — read in its step 1.2; `hooks/new/` holds `pv-new`'s one — `20-after-entry.md` — read in its step 0.3. `scaffold-project.py` seeds every set empty from the start).
...
**Convention: every `pv-*`/`pv-internal-*` `SKILL.md` states its own language rule up front.** Immediately after the skill's lead paragraph (the prose right under the `# skill-name` H1) — before any other bold-labeled scope paragraph, and before the first `##` heading — comes a **`**Language.**`** paragraph stating exactly which `framework.*.language` field(s) govern what that skill writes/says, with its default and fallback chain, and calling out any fixed-language exception (`docs.tech`, `stuff/hooks/*/*.md`, a template's `[[[...]]]`-marked headings). A skill that writes nothing and never talks to the user still states that explicitly here (e.g. `pv-internal-doc-files`, `pv-internal-tech-risks`), rather than omitting the paragraph — so a reader never has to infer language behavior from scattered mentions further down the file. This is a SKILL.md-authoring convention, not a `pv-context.json` schema field: nothing enforces the exact paragraph position automatically (not even `pv-update`'s audit), so it's on whoever writes or reviews a `pv-*` skill to place it there.
...
It also detects the **one-shot migration** of `plan.md`'s `**Risk**` header field to `.metadata.json.risk` (`risk-in-plan-header:*`): it walks `plan.md` under `inProgress/`, `implemented/`, and `closed/`, and for each one that still carries `**Risk**: {median}/10` in the header **and** whose sibling `.metadata.json` doesn't yet have a valid `risk`, it emits the problem. `pv-update` repairs it on its own, without asking (it's part of the normal fix loop): it merges `{"risk": N}` into the `.metadata.json` (preserving `flags`/`flagsLastModified`, creating it if absent) and, **only for `inProgress/` and `implemented/`**, deletes the `- **Risk**: ...` line from the header; the `closed/` `plan.md` is left untouched (frozen history). Idempotent: once `.metadata.json` has a `risk`, it stops being detected.

It also checks the seed hook files of every hook-exposing skill under `{workFolder}/stuff/hooks/<subdir>/` (`version` → `pv-version`, `do` → `pv-do`, `how` → `pv-how`, `new` → `pv-new`): a missing seed (`stuff-<subdir>-hook-missing:<NN>`, `optional`) is recreated by re-running `scaffold-project.py` (copies the owning skill's `<NN>-<slug>.template.md`, header + zero steps, never overwriting); a file with the right `<NN>` id but the wrong slug (`stuff-<subdir>-hook-badslug:<file>`) is renamed to the canonical name — this is also the migration path when the framework renames an insertion point (e.g. `do/10-before-start` → `10-before-implementation`, the `version/` `pre-`/`post-` slugs → `before-`/`after-`), contents kept so project steps survive. Only `version` also has a pre-hooks single-file pipeline form (`{workFolder}/stuff/custom-version-pipeline.md`, or the intermediate `stuff/hooks/custom-version-pipeline.md`), handled by step count: with no `### Step` under any section it's an untouched seed (`stuff-pipeline-legacy-obsolete`) — auto-deleted, then `scaffold-project.py` reseeds the new layout; with real steps (`stuff-pipeline-legacy-location`) `pv-update` does **not** migrate it — the fourth non-auto-fixed exception — it reports the `## section → NN-slug.md` mapping for the user to move the blocks. While that legacy file is present, `version`'s own seed checks are skipped that pass (other sets like `do` still run). The legacy file is read only to count `### Step` headings; the per-point hook files' contents are never inspected.

## Workflow diagrams

Some `pv-*` skills have a flow with several steps and branches (`pv-init`, `pv-update`, `pv-fix`, `pv-how`, `pv-new`, `pv-do`, `pv-version`). For those, the sequence and the branches live in a dedicated Mermaid file next to the skill's `SKILL.md`, and the skill reads it **first**, before executing any step, following it as the source of truth rather than improvising the flow from the prose alone.

**Hook nodes**: a skill with project hook insertion points (`pv-version`, `pv-do`, `pv-how`, `pv-new` — the hook files under `{workFolder}/stuff/hooks/<subdir>/`; plus `pv-fix`, whose fast-track branch reuses `pv-do`'s `stuff/hooks/do/` set) draws each hook point in orange via `classDef hook fill:#d9770e,color:#fff`, applied to both the "does this hook define steps?" decision node and the "run those steps" node, and adds one legend line explaining the color. This is the same orange used for the extension point in `pv-version`'s user-facing `version-flow-diagram.template.md`.
...
**File name and location**: `workflow.<flow-name>.md`, inside the skill's own folder (e.g. `pv-init/workflow.init.md`, `pv-update/workflow.audit.md`, `pv-update/workflow.install.md`, `pv-fix/workflow.fix.md`, `pv-how/workflow.how.md`, `pv-new/workflow.new.md`, `pv-do/workflow.do.md`, `pv-version/workflow.version.md`) — no need to repeat the skill name in the file, the containing folder already indicates it. A skill can have more than one file of this kind if its `SKILL.md` covers several independent entries/flows (`pv-update` is the example: `workflow.audit.md` for the default audit mode, `workflow.install.md` for the explicit install mode).
...
- `<NN>` — two digits, numeric order = run order within the skill. Multiples of `10` for the "main" points; the gaps (`00`–`09`, `11`–`19`, …) are reserved for intercalating future points without renumbering (`version/05` is the first to use `05`). `<NN>` does **not** encode `before`/`after` — the temporal particle does.
...
| `pv-version` | `version/10-before-version` | step 0.7, after the guardrail, before `XXXX` is resolved |
...
│   │   ├── pv-install-scripts/
│   │   │   └── pv-install-scripts.md  # install.sh/install.ps1 internals (English only)
│   │   └── pv-design-onescript/
...
│       ├── pv-how/                     # plans the technical solution
│       │   └── hooks/                        # NN-slug.template.md seeds copied to stuff/hooks/how/
│       │       ├── 10-before-analysis.template.md
│       │       └── 20-after-plan.template.md
│       ├── pv-new/                     # documents an intentional change
│       │   └── hooks/                        # NN-slug.template.md seed copied to stuff/hooks/new/
│       │       └── 20-after-entry.template.md
│       ├── pv-review-architecture/     # proposes code reorganizations, structure only
...
│       │   ├── how-to-compile.template.md   # template pv-version copies when writing stuff/how-to-compile.md
│       │   └── hooks/                        # NN-slug.template.md seeds scaffold-project.py (pv-init) copies to stuff/hooks/version/
│       │       ├── 05-before-guardrail.template.md
│       │       ├── 10-before-version.template.md
│       │       ├── 20-after-build.template.md
│       │       └── 30-after-changelog.template.md
...
└── previo-sdd/                        # {workFolder} — fixed default, not asked
    ...
    │   ├── how-to-compile.md       # build procedure, written by pv-version (lazy creation)
    │   └── hooks/                          # fixed name — one subfolder per hooked process
    │       ├── version/                    # pv-version's hook files, seeded empty by scaffold-project.py
    │       │   ├── 05-before-guardrail.md  # runs before the implemented/ guardrail (XXXX not resolved yet)
    │       │   ├── 10-before-version.md    # runs after the guardrail, before step 1 (XXXX not resolved yet)
    │       │   ├── 20-after-build.md       # runs after artifacts land in versions/XXXX/files/
    │       │   └── 30-after-changelog.md   # runs after the changelog, before the summary
    │       ├── do/                         # pv-do's hook files (also used by pv-fix's fast-track)
    │       │   ├── 10-before-implementation.md  # runs before any code is edited
    │       │   └── 20-after-implementation.md     # runs after code + docs, before the folder moves
...
- `stuff/how-to-compile.md` is lazily created: it doesn't exist until `pv-version` needs it for the first time (or until a change in the build procedure is reported without requesting a release). The `stuff/hooks/version/*.md` files, by contrast, are seeded by `scaffold-project.py` from the start (header + zero steps, so the mechanism is discoverable); `pv-update` recreates any missing one on an older project. None is overwritten once it has content.
