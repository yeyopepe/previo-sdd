...
- **pv-init** — Inicializa el framework: crea/completa `.claude/pv-context.json` (`framework.workFolder` — fijo en `/previo-sdd`, nunca preguntado, raíz relativa al repo bajo la que el framework gestiona `changes/`, `versions/` y `stuff/`, subcarpetas de nombre fijo que las skills crean por sí mismas —, docs a sincronizar, configuración de idioma) y comprueba que las herramientas de línea de comandos necesarias estén instaladas. En un primer `pv-init`, pregunta siempre el idioma de interacción (`framework.interaction.language`) y, con un sí/no, si el resto de áreas (`changes`, `versions`, `docs.functional`) comparten ese mismo idioma o se configuran una a una; deja constancia del porqué de cada elección en `framework._comments`. `docs.tech` no entra en esa pregunta — es siempre inglés técnico. Si el proyecto ya estaba inicializado sin idioma configurado (`hasLanguage: false`), añade esa pregunta a la misma ronda que complete el resto de opcionales pendientes, sin volver a preguntar si ya estaba resuelto. Las tres rutas de documentación (`docs.functional.featuresDocPathDir`, `docs.tech.architectureDocDir`, `docs.tech.styleBibleDocDir`) no están entre los campos "opcionales" que puede dejar sin poner — siempre se escriben y se hace su scaffold; nunca se pregunta si el proyecto quiere esa documentación. Único punto de configuración del que dependen todas las demás skills. *Usa:* ninguna otra skill.
  ...
  - [`scripts/scaffold-project.py`](skills/pv-init/scripts/scaffold-project.py) — crea la estructura base de carpetas (`changes/{inProgress,implemented,todo,closed}`, `versions/`, `stuff/`) y los placeholders de `docs.tech.*`/`docs.functional.featuresDocPathDir` que falten, y sobrescribe `pv.py` en la raíz del repo con la copia de `assets/pv.py`. También escribe `stuff/custom-version-pipeline.md` desde su semilla (las tres secciones normativas `## Before starting` / `## In the middle` / `## At the end`, cero pasos) **solo si no existe** — nunca lo sobrescribe, así que un proyecto que ya añadió pasos los conserva; misma idea que `docs/*` arrancando con su `INDEX.md`/`001-overview.md` en vez de vacío. La semilla `CUSTOM_PIPELINE_SEED` del script es copia byte a byte de [`custom-version-pipeline.template.md`](skills/pv-version/custom-version-pipeline.template.md) de `pv-version` (igual que `NAMESPACE_SEED` ↔ `00-namespace.md`).
...
- **pv-version** — Prepara una entrega en `{workFolder}/versions/{XXXX}/`: exige primero que `{changesDir}/implemented/` esté vacío (cada entrada se resuelve moviéndola a `closed`), genera el entregable siguiendo `{workFolder}/stuff/how-to-compile-version.md` (procedimiento propio del proyecto, escrito la primera vez que hace falta, capaz de describir varios pasos si el build genera varios artefactos), comprime en `.zip` y copia `docs.tech.architectureDocDir`/`docs.tech.styleBibleDocDir`/`docs.functional.featuresDocPathDir` que estén configuradas, y encadena `pv-internal-changelog` para el changelog funcional. Si se invoca solo para informar de un cambio en el procedimiento de build, actualiza `{workFolder}/stuff/how-to-compile-version.md` sin lanzar el resto del proceso salvo confirmación explícita. `{XXXX}` es texto libre elegido por el usuario en cada invocación, sin relación con la numeración `xxxx` de change/fix ni con ninguna otra carpeta "versions" que exista en el repo. *Usa:* `pv-internal-changelog`.
  ...
  **Puntos de personalización (no se edita la skill).** El `SKILL.md`, `workflow.version.md` y demás ficheros bajo `.claude/skills/pv-*/` son framework instalado, gestionado vía `pv-update`; editarlos a mano los deja inconsistentes. Hay exactamente dos puntos de personalización, ambos en `{workFolder}/stuff/`: cómo compilar el entregable → `how-to-compile-version.md` (pasos 3–4); los pasos propios del proyecto en tres momentos del flujo → `custom-version-pipeline.md` (paso 0.6). Este segundo fichero tiene tres secciones normativas de nombre fijo — `## Before starting` (antes del paso 1, solo `{workFolder}` sustituido; `{XXXX}` aún no resuelto), `## In the middle` (paso 4.1, tras copiar los artefactos a `files/`; `{XXXX}` y las rutas `versions/{XXXX}/` disponibles) y `## At the end` (paso 6.1, tras el changelog y antes del resumen; mismas variables) —, cada una con 0..N bloques `### Step N: {name}` de la misma forma que `how-to-compile-version.md`. Una sección sin pasos se salta en silencio; si un paso falla, el flujo se detiene y se explica (mismo criterio que el paso 4). El resumen final (paso 7) indica qué secciones se ejecutaron y qué produjeron. Si lo pedido por el usuario no encaja en ninguno de los dos ficheros, se le dice y se propone abrir un cambio en el repo del framework — nunca un parche local a la skill.

  Assets y scripts:
  - [`workflow.version.md`](skills/pv-version/workflow.version.md) — diagrama Mermaid del flujo completo de esta skill (ver "Diagramas de flujo" más arriba); se lee antes de ejecutar cualquier paso, fuente de verdad de la secuencia y las ramas. Incluye los tres puntos de enganche del pipeline personalizado (`Before starting` antes de resolver `XXXX`, `In the middle` tras copiar artefactos, `At the end` tras el changelog). No confundir con `version-flow-diagram.template.md` de abajo: este es el flujo interno completo (cada nodo `ASK`/`INFO` y rama), el otro es una vista simplificada orientada al usuario.
  ...
  - [`custom-version-pipeline.template.md`](skills/pv-version/custom-version-pipeline.template.md) — semilla de `{workFolder}/stuff/custom-version-pipeline.md`: solo las tres secciones normativas de nombre fijo (`## Before starting` / `## In the middle` / `## At the end`) y cero pasos. La crea `scaffold-project.py` de `pv-init` (no `pv-version`), que la copia byte a byte; `pv-version` (paso 0.6) localiza esos tres encabezados literalmente y ejecuta los `### Step N` que cada uno contenga. Nunca se sobrescribe una vez tiene contenido.
  ...
  - [`version-flow-diagram.template.md`](skills/pv-version/version-flow-diagram.template.md) — diagrama Mermaid general y simplificado del proceso de `pv-version` (guardarraíl de `implemented/` vacío → pasos propios del proyecto (`stuff/custom-version-pipeline.md`) → crear carpeta → compilar → comprimir docs → changelog → confirmar), pensado para mostrarse tal cual si el usuario pregunta cómo funciona `/pv-version` — solo la información relevante y comprensible para el usuario, no todo el ramificado interno que sí cubre `workflow.version.md`. El nodo del pipeline personalizado va en naranja (`classDef extension`), distinto de los pasos mecánicos azules.
...
- **`workFolder`** (`string`, opcional, default `"/previo-sdd"`): carpeta relativa a la raíz del repo bajo la que el framework gestiona todo su trabajo. La `/` inicial es solo un convenio visual (para que se distinga a simple vista de `docs.*`, que es relativa a `workFolder` mismo) — es opcional y todo script `pv-*` la quita antes de resolver la ruta, así que `"previo-sdd"` y `"/previo-sdd"` funcionan igual; nunca es una ruta absoluta real del sistema de ficheros. Es el único campo de `framework` que `pv-init` nunca pregunta ni confirma: siempre escribe el default en silencio, igual que `skills.mockups`/`skills.diagrams`. Si algún día se quiere otra carpeta, se cambia a mano en `pv-context.json`, bajo la responsabilidad de quien lo edite. Se resuelve vía `resolve-path.py` — ver "Resolución de rutas" más abajo. Dentro de ella, `scaffold-project.py` de `pv-init` crea tres subcarpetas de nombre fijo justo después de escribir `pv-context.json`, que el usuario no elige ni renombra:
  ...
  - `{workFolder}/stuff/` — ficheros propios del proyecto que ninguna otra skill del framework decide por él: `how-to-compile-version.md` (procedimiento de build que `pv-version` pregunta y escribe la primera vez que hace falta) y `custom-version-pipeline.md` (pasos propios del proyecto en tres momentos del flujo de `pv-version` — lo crea `scaffold-project.py` con sus tres secciones vacías desde el arranque, y `pv-version` lo lee en el paso 0.6).
...
Comprueba también que exista la semilla de `pv-version`: si `{workFolder}/stuff/` está presente pero no tiene `custom-version-pipeline.md` (`stuff-custom-pipeline-missing`, severidad `optional`) — un proyecto generado antes de que ese fichero se añadiera —, `pv-update` re-ejecuta `scaffold-project.py`, que lo recrea desde su semilla (tres secciones, cero pasos) sin sobrescribir nada existente. Solo se comprueba la presencia del fichero, nunca su contenido — misma familia de chequeo que `*-missing-index` / `namespace-missing`: se recrea la semilla, nunca se inventan pasos.
...
│       │   └── custom-version-pipeline.template.md  # semilla que scaffold-project.py (pv-init) copia a stuff/custom-version-pipeline.md
...
└── previo-sdd/                        # {workFolder} — default fijo, no se pregunta
    ...
    │   └── custom-version-pipeline.md      # pasos propios del proyecto en 3 puntos del flujo de pv-version; sembrado vacío por scaffold-project.py
    │
...
- `stuff/how-to-compile-version.md` es de creación perezosa: no existe hasta que `pv-version` lo necesita por primera vez (o hasta que se reporta un cambio en el procedimiento de build sin pedir entrega). `stuff/custom-version-pipeline.md`, en cambio, lo siembra `scaffold-project.py` desde el arranque con sus tres secciones vacías (para que el mecanismo sea descubrible); `pv-update` lo recrea si falta en un proyecto antiguo. Ninguno de los dos se sobrescribe una vez tiene contenido.
