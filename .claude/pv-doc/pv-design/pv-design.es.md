...
- **pv-init** — Inicializa el framework: crea/completa `.claude/pv-context.json` (`framework.workFolder` — fijo en `/previo-sdd`, nunca preguntado, raíz relativa al repo bajo la que el framework gestiona `changes/`, `versions/` y `stuff/`, subcarpetas de nombre fijo que las skills crean por sí mismas —, docs a sincronizar, configuración de idioma) y comprueba que las herramientas de línea de comandos necesarias estén instaladas. En un primer `pv-init`, pregunta siempre el idioma de interacción (`framework.interaction.language`) y, con un sí/no, si el resto de áreas (`changes`, `versions`, `docs.functional`) comparten ese mismo idioma o se configuran una a una; deja constancia del porqué de cada elección en `framework._comments`. `docs.tech` no entra en esa pregunta — es siempre inglés técnico. Si el proyecto ya estaba inicializado sin idioma configurado (`hasLanguage: false`), añade esa pregunta a la misma ronda que complete el resto de opcionales pendientes, sin volver a preguntar si ya estaba resuelto. Las tres rutas de documentación (`docs.functional.featuresDocPathDir`, `docs.tech.architectureDocDir`, `docs.tech.styleBibleDocDir`) no están entre los campos "opcionales" que puede dejar sin poner — siempre se escriben y se hace su scaffold; nunca se pregunta si el proyecto quiere esa documentación. Único punto de configuración del que dependen todas las demás skills. *Usa:* ninguna otra skill.
  ...
  - [`scripts/scaffold-project.py`](skills/pv-init/scripts/scaffold-project.py) — crea la estructura base de carpetas (`changes/{inProgress,implemented,todo,closed}`, `versions/`, `stuff/`, `stuff/hooks/`) y los placeholders de `docs.tech.*`/`docs.functional.featuresDocPathDir` que falten, y sobrescribe `pv.py` en la raíz del repo con la copia de `assets/pv.py`. También crea `stuff/hooks/version/` y copia dentro los tres ficheros de hook de `pv-version` ([`hooks/10-pre-release.template.md`](skills/pv-version/hooks/10-pre-release.template.md), `20-post-build`, `30-post-changelog`) como `NN-slug.md` **solo si cada uno no existe** — nunca sobrescribe ninguno, así que un proyecto que ya añadió pasos los conserva; misma idea que `docs/*` arrancando con su `INDEX.md`/`001-overview.md` en vez de vacío. A diferencia de `NAMESPACE_SEED` (embebida en el script), las semillas de hook se leen de los `.template.md` en tiempo de ejecución (como `assets/pv.py`).
...
- **`workFolder`** (`string`, opcional, default `"/previo-sdd"`): carpeta relativa a la raíz del repo bajo la que el framework gestiona todo su trabajo. La `/` inicial es solo un convenio visual (para que se distinga a simple vista de `docs.*`, que es relativa a `workFolder` mismo) — es opcional y todo script `pv-*` la quita antes de resolver la ruta, así que `"previo-sdd"` y `"/previo-sdd"` funcionan igual; nunca es una ruta absoluta real del sistema de ficheros. Es el único campo de `framework` que `pv-init` nunca pregunta ni confirma: siempre escribe el default en silencio, igual que `skills.mockups`/`skills.diagrams`. Si algún día se quiere otra carpeta, se cambia a mano en `pv-context.json`, bajo la responsabilidad de quien lo edite. Se resuelve vía `resolve-path.py` — ver "Resolución de rutas" más abajo. Dentro de ella, `scaffold-project.py` de `pv-init` crea tres subcarpetas de nombre fijo justo después de escribir `pv-context.json`, que el usuario no elige ni renombra:
  ...
  - `{workFolder}/stuff/` — ficheros propios del proyecto que ninguna otra skill del framework decide por él: `how-to-compile.md` (procedimiento de build que `pv-version` pregunta y escribe la primera vez que hace falta) y `hooks/` (una subcarpeta por proceso con hooks; `hooks/version/` contiene los tres ficheros de hook de `pv-version` — `scaffold-project.py` los siembra vacíos desde el arranque, y `pv-version` los lee en el paso 0.6).
...
Comprueba también los ficheros de hook de `pv-version` bajo `{workFolder}/stuff/hooks/version/`: una semilla que falte (`stuff-version-hook-missing:<NN>`, `optional`) se recrea re-ejecutando `scaffold-project.py` (copia el `<NN>-<slug>.template.md`, encabezado + cero pasos, sin sobrescribir); un fichero con el id `<NN>` correcto pero el slug equivocado (`stuff-version-hook-badslug:<file>`) se renombra al nombre canónico. Un pipeline de fichero único pre-hooks (`{workFolder}/stuff/custom-version-pipeline.md`, o el intermedio `stuff/hooks/custom-version-pipeline.md`) se trata según cuántos pasos tenga: sin ningún `### Step` bajo ninguna sección es una semilla intacta (`stuff-pipeline-legacy-obsolete`) — se borra automáticamente y `scaffold-project.py` resiembra la nueva estructura; con pasos reales (`stuff-pipeline-legacy-location`) `pv-update` **no** lo migra — el cuarto caso no auto-corregible —, reporta el mapeo `## sección → NN-slug.md` para que la persona usuaria mueva los bloques. El fichero antiguo se lee solo para contar encabezados `### Step`; el contenido de los ficheros de hook por punto no se inspecciona nunca.
