...
- **pv-init** — Inicializa el framework: crea/completa `.claude/pv-context.json` (`framework.workFolder` — fijo en `/previo-sdd`, nunca preguntado, raíz relativa al repo bajo la que el framework gestiona `changes/`, `versions/` y `stuff/`, subcarpetas de nombre fijo que las skills crean por sí mismas —, docs a sincronizar, configuración de idioma) y comprueba que las herramientas de línea de comandos necesarias estén instaladas. Antes de tocar cualquier configuración, su paso 0 verifica las herramientas de desarrollo que el propio framework necesita (Git, Python 3) más las que añada el tipo de proyecto detectado (p. ej. Node/npm si hay `package.json`), pidiendo confirmación antes de instalar nada y dejando que el usuario siga igualmente o se detenga si algo no se puede instalar. En un primer `pv-init`, pregunta siempre el idioma de interacción (`framework.interaction.language`) y, con un sí/no, si el resto de áreas (`changes`, `versions`, `docs.functional`) comparten ese mismo idioma o se configuran una a una; deja constancia del porqué de cada elección en `framework._comments`. `docs.tech` no entra en esa pregunta — es siempre inglés técnico. Si el proyecto ya estaba inicializado sin idioma configurado (`hasLanguage: false`), añade esa pregunta a la misma ronda que complete el resto de opcionales pendientes, sin volver a preguntar si ya estaba resuelto. Las tres rutas de documentación (`docs.functional.featuresDocPathDir`, `docs.tech.architectureDocDir`, `docs.tech.styleBibleDocDir`) no están entre los campos "opcionales" que puede dejar sin poner — siempre se escriben y se hace su scaffold; nunca se pregunta si el proyecto quiere esa documentación. Si `sourcecodeDir` ya contiene código real (no solo un scaffold vacío) en el momento del init, pregunta al usuario qué profundidad de documentación quiere — **mínima** (más rápida, menor consumo de tokens, mejora con el tiempo según se añaden changes/fixes) o **completa** (más lenta, mayor consumo de tokens, resultado completo desde el principio) — y, una vez escrita el resto de la configuración, invoca `pv-internal-tech-analysis` (con `bootstrap: true`, para que explore el código real en vez de resolver el `docs.tech` todavía en placeholder) para convertir los tres placeholders sembrados en documentación inicial real a esa profundidad: contenido de arquitectura más un mapa de ficheros/símbolos, una guía de estilo (solo si el proyecto tiene capa de presentación — una GUI o una CLI cuentan), el listado exhaustivo de funcionalidades, y la semilla `00-namespace.md` ya poblada. Se omite por completo si no hay código existente que analizar. Único punto de configuración del que dependen todas las demás skills. *Usa:* ninguna otra skill.
  ...
  - [`scripts/scaffold-project.py`](skills/pv-init/scripts/scaffold-project.py) — crea la estructura base de carpetas (`changes/{inProgress,implemented,todo,closed}`, `versions/`, `stuff/`, `stuff/hooks/`) y los placeholders de `docs.tech.*`/`docs.functional.featuresDocPathDir` que falten, y sobrescribe `pv.py` en la raíz del repo con la copia de `assets/pv.py`. También crea una carpeta `stuff/hooks/<subdir>/` por cada skill que expone hooks (ver su `HOOK_SETS`) y copia dentro los ficheros de hook de esa skill como `NN-slug.md` **solo si cada uno no existe** — `stuff/hooks/version/` recibe los cuatro de `pv-version` ([`hooks/05-before-guardrail.template.md`](skills/pv-version/hooks/05-before-guardrail.template.md), `10-before-version`, `20-after-build`, `30-after-changelog`), `stuff/hooks/do/` recibe los dos de `pv-do` ([`hooks/10-before-implementation.template.md`](skills/pv-do/hooks/10-before-implementation.template.md), `20-after-implementation`), `stuff/hooks/how/` recibe los dos de `pv-how` ([`hooks/10-before-analysis.template.md`](skills/pv-how/hooks/10-before-analysis.template.md), `20-after-plan`), `stuff/hooks/new/` recibe el de `pv-new` ([`hooks/20-after-entry.template.md`](skills/pv-new/hooks/20-after-entry.template.md)). Nunca sobrescribe ninguno, así que un proyecto que ya añadió pasos los conserva; misma idea que `docs/*` arrancando con su `INDEX.md`/`001-overview.md` en vez de vacío. A diferencia de `NAMESPACE_SEED` (embebida en el script), las semillas de hook se leen de los `.template.md` en tiempo de ejecución (como `assets/pv.py`).
...
- **pv-do** — Toma una entrada de `inProgress` cuyo `plan.md` ya está escrito (por `pv-how`, o invocada directamente por el usuario), implementa el código, actualiza la documentación sincronizada (`docs.tech.architectureDocDir`/`docs.functional.featuresDocPathDir`/`docs.tech.styleBibleDocDir` — incluyendo cualquier incongruencia que `pv-internal-tech-analysis` haya reportado vía `pv-how`) y mueve la carpeta a `implemented` vía `pv-internal-workflow`. Si `docs.functional.featuresDocPathDir` es una carpeta, delega en `pv-internal-doc-features` tanto la lectura/escritura como la decisión de qué contenido lleva la entrada y cómo redactarla (`pv-do` solo aporta resumen y contexto). Antes de redactar o editar contenido de `docs.tech.architectureDocDir`/`styleBibleDocDir`, invoca `pv-internal-doc-technical` para cargar su estilo de escritura (pensado para que lo lea una IA, no una persona) y lo aplica al redactar; para `styleBibleDocDir` en concreto, invoca además `pv-internal-doc-style` para obtener las categorías de estilo aplicables y sus propias reglas de escritura. Redacta ambos siempre en **inglés técnico** (no hay `docs.tech.language`), traduciendo desde `plan.md` si venía en otro idioma. Cuando el cambio añade o renombra un concepto/afirmación citable, mantiene `{architectureDocDir}/00-namespace.md` editándolo con Read/Edit directo (nunca vía `upsert`). *Usa:* `pv-internal-workflow`, `pv-internal-doc-features`, `pv-internal-doc-technical`, `pv-internal-doc-style`.
  ...
  **Puntos de personalización (no se edita la skill).** El `SKILL.md`, `workflow.do.md` y demás ficheros bajo `.claude/skills/pv-do/` son framework instalado, gestionado vía `pv-update`. El único punto de personalización es `{workFolder}/stuff/hooks/do/`, un fichero por punto de inserción: `10-before-implementation.md` (paso 2, antes de editar código) y `20-after-implementation.md` (final del paso 2.1, tras código + docs, antes de mover la carpeta a `implemented/`). Cada uno con 0..N bloques `### Step N: {name}` de la misma forma que `how-to-compile.md`; `pv-do` casa un fichero por su prefijo de id `NN-` (el `slug` también es fijo). Ambos hooks sustituyen `{workFolder}` y `{xxxx}`. Un fichero sin pasos, o ausente, se salta en silencio; si un paso falla, el flujo se detiene y se explica, y la carpeta no se mueve. `scaffold-project.py` siembra los dos ficheros vacíos; `pv-do` los lee en el paso 1.5. El subflujo rápido de `pv-fix` lee y ejecuta esos mismos dos ficheros (su paso 2, luego puntos de hook en los pasos 3 y 6) — edita código sin pasar por `pv-do`, así que aplican los mismos hooks antes/después; no hay un conjunto `fix/` aparte.

  Assets y scripts:
  - [`workflow.do.md`](skills/pv-do/workflow.do.md) — diagrama Mermaid del flujo completo de esta skill (ver "Diagramas de flujo" más arriba); se lee antes de ejecutar cualquier paso, fuente de verdad de la secuencia y las ramas. Incluye los dos puntos de hook (`10-before-implementation` antes de implementar, `20-after-implementation` tras código + docs y antes de mover la carpeta), dibujados en naranja.
  - [`hooks/*.template.md`](skills/pv-do/hooks/) — las semillas para `{workFolder}/stuff/hooks/do/<NN>-<slug>.md`, una por punto de inserción (`10-before-implementation`, `20-after-implementation`): un encabezado que explica cuándo se ejecuta el hook (tanto desde `pv-do` como desde la vía rápida de `pv-fix`) y qué variables hay disponibles, más un esqueleto `### Step N` comentado, cero pasos vivos. El `scaffold-project.py` de `pv-init` las copia (no `pv-do`), cada una solo si no existe; `pv-do` (paso 1.5) y la vía rápida de `pv-fix` (paso 2) casan un fichero por su prefijo de id `<NN>` y ejecutan los bloques `### Step N` que contenga. Nunca se sobrescriben una vez tienen contenido.
...
- **pv-review-doc-tech** — Reorganiza, solo estructura, cada carpeta configurada bajo `framework.docs.tech` (hoy `architectureDocDir` y `styleBibleDocDir`; lee dinámicamente las claves presentes ahí en vez de fijar las dos por nombre, así una futura tercera carpeta se recoge sin cambiar la skill). Nunca borra un hecho, reescribe una frase ni añade contenido — solo lo mueve al fichero/`**Area**`/forma de notación que ya debería tener según las mismas reglas que aplica `pv-do` al crear esa documentación: un fichero por tema (convención `find`/`upsert` de `pv-internal-doc-files`), los checklists de categorías de `pv-internal-doc-technical`/`pv-internal-doc-style` para juzgar a qué pertenece el contenido, y las reglas de escritura notation-first de `pv-internal-doc-technical` para juzgar el desorden interno de un fichero. Lee la carpeta entera (`INDEX.md`, cada `{NNN}-{slug}.md` y `00-namespace.md`) antes de decidir nada, así un duplicado repartido entre dos ficheros o una colisión de ruta de namespace no pasan desapercibidos — un pase fichero a fichero por sí solo se los perdería. Nunca renumera/renombra un fichero `{NNN}-{slug}.md`, nunca edita `INDEX.md` a mano (solo lo hace `rebuild-index.py`), y nunca edita `00-namespace.md` directamente — un cambio de namespace que identifique como necesario se lo señala al usuario en vez de aplicarlo. No invoca a `pv-internal-doc-technical`/`pv-internal-doc-style`/`pv-internal-doc-files` como llamadas Skill en vivo (no hay contenido que redactar ni fichero que ubicar que las necesite a mitad de flujo); aplica sus reglas ya documentadas directamente mientras reorganiza, y solo invoca el script `rebuild-index.py` de `pv-internal-doc-files` una vez terminadas las ediciones de una carpeta. *Usa:* ninguna otra skill (aplica las reglas de `pv-internal-doc-technical`/`pv-internal-doc-style`/`pv-internal-doc-files` directamente en vez de invocarlas).
...
- **pv-update** — Audita y repara `.claude/pv-context.json` y todo lo que configura contra el estado real del repo (forma del esquema, claves obsoletas, skills referenciadas, rutas en disco, la semilla de namespace de `architectureDocDir`, las semillas `stuff/hooks/*/*.md`, la frescura de `pv.py`, la deriva de `skillModels`, las etiquetas estructurales marcadas con `[[[...]]]`, códigos de change duplicados, y la consistencia de versión entre todas las skills `pv-*`) — arregla todo lo que puede determinar sin ambigüedad y reporta qué cambió; solo pregunta al usuario cuando `pv-context.json` es JSON roto, la versión instalada parece un downgrade, o el fichero de un ancla de namespace ya no existe. También es el propio mecanismo de instalación/actualización del framework (`/pv-update install`): resuelve una versión objetivo (última release oficial, o un `--version` explícito), rechaza de forma dura un downgrade o un tag no parseable sin flag de escape, avisa (sin bloquear) cuando el tag resuelto coincide exactamente con la versión instalada (una reinstalación desde cero, no una actualización), y solo instala después de que el usuario haya confirmado explícitamente ese tag resuelto exacto por su nombre — forzado por un protocolo de dos pasos (`install-framework.py` primero resuelve y valida, solo instala si se vuelve a invocar con `--yes`) para que esta skill nunca pueda instalar una versión que el usuario no ha confirmado. La opción de menú "Install new Previo version" de `pv.py` aplica la misma regla de downgrade y el mismo aviso de misma-versión, alcanzada desde un punto de entrada distinto (ver [`pv-design-onescript.es.md`](../pv-doc/pv-design-onescript/pv-design-onescript.es.md)) — `pv.py` filtra un tag anterior al instalado directamente de su lista en vez de ofrecerlo y fallar después, ya que solo lista dos tags conocidos (oficial + pre-release) en vez de aceptar input libre del usuario. Este segundo modo nunca se dispara por sí solo; el flujo por defecto, solo auditoría, nunca toca la red ni instala nada. El paso 0 de cualquier otra skill pública `pv-*` ejecuta `check-framework-status.py` (más abajo) como comprobación barata antes de hacer su propio trabajo, y envía al usuario aquí si falla. *Usa:* ninguna otra skill.

  Assets y scripts:
  - [`workflow.audit.md`](skills/pv-update/workflow.audit.md) — diagrama Mermaid del flujo solo-auditoría (ver "Diagramas de flujo" arriba): leer `pv-context.json` a lo mejor posible, correr `audit-context.py`, recorrer cada problema reportado aplicando su arreglo determinista (o preguntando, para las cuatro excepciones que necesitan criterio), escribir la versión verificada, reportar. Fuente de verdad de la secuencia y ramas de este modo.
  - [`scripts/audit-context.py`](skills/pv-update/scripts/audit-context.py) — el auditor completo, de solo lectura: todas las comprobaciones deterministas que corre esta skill, imprimiendo un único JSON de `problems`. No escribe nada; suficientemente lento (recorre todo el árbol `changes/`, plantillas, hooks) como para correr solo bajo demanda, no en cada invocación de otra skill.
  - [`scripts/mark-verified.py`](skills/pv-update/scripts/mark-verified.py) — escribe `framework.frameworkStatus` tras una auditoría: `--clear` (sano, o un caso de desactualización recién resuelto), `--block "<motivo>"` (se detectó un downgrade), `--confirm-downgrade` (el usuario confirmó que era intencional).
  - [`scripts/check-framework-status.py`](skills/pv-update/scripts/check-framework-status.py) — el subconjunto barato que cada otra skill pública `pv-*` corre en cada invocación, no solo `pv-update` misma: sin argumentos, imprime un único JSON (`ok`, `problem`, `message`), parando en el primero de `context-missing` → `context-invalid-json` → `blocked` → `version-mismatch` → `skill-count-mismatch` (el número exacto de carpetas de skills `pv-*` instaladas, `EXPECTED_SKILL_COUNT`, sincronizado por `tools/set-skill-versions.py` en cada corte de release). Deliberadamente un gate binario rápido, no el auditor completo — nunca duplica las comprobaciones caras de `audit-context.py`.
  - [`workflow.install.md`](skills/pv-update/workflow.install.md) — diagrama Mermaid del flujo del modo instalación (ver "Diagramas de flujo" arriba): consultar GitHub Releases para la última versión oficial y cualquier pre-release más reciente, resolver la versión objetivo, rechazar un downgrade o tag no parseable, avisar (sin bloquear) si el tag resuelto coincide exactamente con el instalado o si es una pre-release, pedir al usuario que confirme ese tag resuelto exacto por su nombre, solo entonces volver a invocar `install-framework.py` con `--yes`, reportar y recordar al usuario correr `/pv-update` (modo audit) a continuación.
  - [`scripts/install-framework.py`](skills/pv-update/scripts/install-framework.py) — resuelve un único tag objetivo (`--version` explícito, o la última release oficial) y, sin `--yes`, solo lo valida y se detiene: consulta `GET /repos/{REPO}/releases/latest` y `GET /repos/{REPO}/releases?per_page=1` para informar siempre de la última release oficial y, si es más reciente, una pre-release; rechaza de forma dura, sin flag de escape, un tag no parseable o un downgrade real contra la versión instalada, para ese tag resuelto sin importar si vino de `--version` o de "latest" (un "instala la última" desatendido que downgradearía en silencio una pre-release instalada se rechaza igual que un `--version` anterior explícito). Solo con `--version <tag> --yes` instala de verdad: detecta el SO (`platform.system()`, nunca por qué binarios hay en `PATH`), borra cualquier `install.ps1`/`install.sh` que encuentre en la raíz del repo (nunca se confía en él, aunque esté presente) y descarga una copia limpia desde la rama `main` de previo-sdd a un archivo temporal, lo ejecuta con el tag resuelto, y borra el archivo temporal de nuevo después — sin importar éxito o fallo. Nunca ejecuta `mark-verified.py` por sí misma.

- **pv-version** — Prepara una entrega en `{workFolder}/versions/{XXXX}/`: exige primero que `{changesDir}/implemented/` esté vacío (cada entrada se resuelve moviéndola a `closed`), genera el entregable siguiendo `{workFolder}/stuff/how-to-compile.md` (procedimiento propio del proyecto, escrito la primera vez que hace falta, capaz de describir varios pasos si el build genera varios artefactos), comprime en `.zip` y copia `docs.tech.architectureDocDir`/`docs.tech.styleBibleDocDir`/`docs.functional.featuresDocPathDir` que estén configuradas, y encadena `pv-internal-changelog` para el changelog funcional. Si se invoca solo para informar de un cambio en el procedimiento de build, actualiza `{workFolder}/stuff/how-to-compile.md` sin lanzar el resto del proceso salvo confirmación explícita. `{XXXX}` es texto libre elegido por el usuario en cada invocación, sin relación con la numeración `xxxx` de change/fix ni con ninguna otra carpeta "versions" que exista en el repo. *Usa:* `pv-internal-changelog`.

  **Puntos de personalización (no se edita la skill).** El `SKILL.md`, `workflow.version.md` y demás ficheros bajo `.claude/skills/pv-*/` son framework instalado, gestionado vía `pv-update`; editarlos a mano los deja inconsistentes. Hay exactamente dos puntos de personalización en `{workFolder}/stuff/`: cómo compilar el entregable → `how-to-compile.md` (pasos 3–4); los pasos propios del proyecto en cuatro momentos del flujo → los ficheros de hook bajo `hooks/version/` (paso 0.6), uno por punto: `05-before-guardrail.md` (paso 0.4, antes del guardarraíl de `implemented/` y del paso 1, solo `{workFolder}` sustituido; `{XXXX}` aún no resuelto), `10-before-version.md` (paso 0.7, tras el guardarraíl, antes del paso 1, mismas variables), `20-after-build.md` (paso 4.1, tras copiar los artefactos a `files/`; `{XXXX}` y las rutas `versions/{XXXX}/` disponibles) y `30-after-changelog.md` (paso 6.1, tras el changelog y antes del resumen; mismas variables). Cada uno con 0..N bloques `### Step N: {name}` de la misma forma que `how-to-compile.md`; `pv-version` casa un fichero por su prefijo de id `NN-` (el `slug` también es fijo). Un fichero sin pasos, o ausente, se salta en silencio; si un paso falla, el flujo se detiene y se explica (mismo criterio que el paso 4). El resumen final (paso 7) indica qué hooks se ejecutaron y qué produjeron. Si lo pedido por el usuario no encaja en ninguno de los puntos, se le dice y se propone abrir un cambio en el repo del framework — nunca un parche local a la skill.

  Assets y scripts:
  - [`workflow.version.md`](skills/pv-version/workflow.version.md) — diagrama Mermaid del flujo completo de esta skill (ver "Diagramas de flujo" más arriba); se lee antes de ejecutar cualquier paso, fuente de verdad de la secuencia y las ramas. Incluye los cuatro puntos de enganche (`05-before-guardrail` antes del guardarraíl de `implemented/`, `10-before-version` tras el guardarraíl y antes de resolver `XXXX`, `20-after-build` tras copiar artefactos, `30-after-changelog` tras el changelog). No confundir con `version-flow-diagram.template.md` de abajo: este es el flujo interno completo (cada nodo `ASK`/`INFO` y rama), el otro es una vista simplificada orientada al usuario.
  - [`how-to-compile.template.md`](skills/pv-version/how-to-compile.template.md) — plantilla que `pv-version` copia a `{workFolder}/stuff/how-to-compile.md` la primera vez que hace falta: comando(s) a ejecutar, fichero(s) generado(s) y notas, con soporte para varios pasos independientes si el build genera varios artefactos.
  - [`hooks/*.template.md`](skills/pv-version/hooks/) — semillas de `{workFolder}/stuff/hooks/version/<NN>-<slug>.md`, una por punto de inserción (`05-before-guardrail`, `10-before-version`, `20-after-build`, `30-after-changelog`): un encabezado que explica cuándo corre el hook y qué variables están disponibles, más un esqueleto `### Step N` comentado, cero pasos vivos. Las copia `scaffold-project.py` de `pv-init` (no `pv-version`), cada una solo si falta; `pv-version` (paso 0.6) casa un fichero por su prefijo de id `<NN>` y ejecuta los bloques `### Step N` que contenga. Nunca se sobrescribe una vez tiene contenido.
  ...
  - [`version-flow-diagram.template.md`](skills/pv-version/version-flow-diagram.template.md) — diagrama Mermaid general y simplificado del proceso de `pv-version` (paso de hook propio del proyecto `05-before-guardrail` → guardarraíl de `implemented/` vacío → pasos de hook propios del proyecto `10`/`20`/`30` (`stuff/hooks/version/*.md`) → crear carpeta → compilar → comprimir docs → changelog → confirmar), pensado para mostrarse tal cual si el usuario pregunta cómo funciona `/pv-version` — solo la información relevante y comprensible para el usuario, no todo el ramificado interno que sí cubre `workflow.version.md`. Los nodos de hook van en naranja (`classDef extension`), distintos de los pasos mecánicos azules.
...
- **pv-internal-mockups-html** — Genera o edita maquetas visuales estáticas en HTML/CSS/SVG autocontenido (`design_*.html`) de un elemento de UI nuevo o modificado, a partir de la carpeta destino y la lista de elementos que quien invoca necesita maquetar, leyendo antes el style bible del proyecto (`docs.tech.styleBibleDocDir`, solo lectura) para que la maqueta replique la identidad visual documentada en vez de una inventada. Si el style bible no cubre un elemento, no cae directamente a un estilo neutro de relleno: busca el estilo real ya presente en `sourcecodeDir` (ficheros de tema/tokens, hojas de estilo, el componente existente más parecido — solo lectura, igual que el style bible) y lo reutiliza. En cualquiera de los dos casos el hueco es real, así que lo reporta a quien la invoca como una lista de **huecos de estilo encontrados** (qué se reutilizó del código y dónde), para que quien la invoca pueda señalar la documentación que falta en vez de que pase desapercibido. No decide qué elementos hacen falta ni valida nada con el usuario, solo produce los ficheros y devuelve sus rutas más esa lista de huecos. Es la skill de maquetas por defecto de `framework.skills.mockups`. La usan `pv-new` y `pv-fix`. *Usa:* ninguna otra skill.

  Assets y scripts:
  - [`assets/mockup-annotations.html`](skills/pv-internal-mockups-html/assets/mockup-annotations.html) — la copia maestra del **mnoteqz7k-framework**, un runtime de anotaciones de revisión autocontenido (barra flotante, notas ancladas a elementos, paneles de notas generales/vinculadas, mostrar/ocultar, guardado vía la File System Access API con fallback a descarga por Blob). Todo su namespace (ids, clases, el comentario marcador) lleva el prefijo `mnoteqz7k` en vez del más obvio `mnote`, a propósito — una etiqueta corta y efectivamente aleatoria hace improbable una colisión accidental con el naming propio de un mockup real, algo que importa porque el trabajo del ID collision guard es precisamente distinguir "el framework ya está incrustado" de "un elemento sin relación que coincide de nombre por casualidad". Sin cuerpo de demo a propósito — solo el marcador `<!-- mnoteqz7k-framework vN -->`, `<style id="mnoteqz7k-styles">`, un `#mnoteqz7k-data` vacío y `<script id="mnoteqz7k-runtime">`, los bloques exactos que `pv-internal-mockups-html` debe copiar verbatim en cada `design_*.html` que genere (ese empalme todavía no está implementado en `SKILL.md` — ver `.claude/plans/interactive-mockups/PLAN.md`, en implementación parcial a fecha de esta nota).
  - [`assets/SANDBOX.md`](skills/pv-internal-mockups-html/assets/SANDBOX.md) — instrucciones, solo para desarrollo, de cómo probar manualmente el mnoteqz7k-framework contra una gran variedad de widgets interactivos y DOM anidado complejo (un script generador aparte más un par sandbox de trabajo/golden, para que las pruebas nunca contaminen el asset maestro). `install.sh`/`install.ps1` eliminan cualquier fichero `*.sandbox.*` y el script generador de lo que copian a un proyecto consumidor — `SANDBOX.md` y el asset maestro son los únicos ficheros de esta carpeta pensados para distribuirse.

- **pv-internal-mockups-ascii** — Misma función y mismo contrato de entrada/salida que `pv-internal-mockups-html` (incluido el mismo fallback style-bible-luego-código y el reporte de **huecos de estilo encontrados**, aplicado a layout y microcopy en vez de estilo visual), pero generando las maquetas como arte ASCII en texto plano (`design_*.txt`) en vez de HTML. Solo se invoca cuando un proyecto configura `framework.skills.mockups` para usar esta alternativa en lugar de la de por defecto. *Usa:* ninguna otra skill.
...
**pv-review-doc-tech** (user-invocable, listada más arriba) no aparece en este diagrama — nunca invoca a `pv-do` ni a estas cuatro skills como proceso en vivo. Es un segundo consumidor, separado, de las mismas dos skills de checklist de contenido (`pv-internal-doc-technical`/`pv-internal-doc-style`): donde `pv-do` las llama para decidir **qué contenido nuevo escribir**, `pv-review-doc-tech` aplica sus categorías y reglas de escritura **como estándar fijo contra el que reorganizar contenido ya existente**, sin llamarlas a mitad de flujo (no hay nada que puedan devolver que cambie un movimiento estructural ya decidido a partir de leer el checklist). Sí invoca directamente el script `rebuild-index.py` de `pv-internal-doc-files`, el mismo paso de regeneración que dispara la propia acción `upsert` de `doc-files`.
...
- **`workFolder`** (`string`, opcional, default `"/previo-sdd"`): carpeta relativa a la raíz del repo bajo la que el framework gestiona todo su trabajo. La `/` inicial es solo un convenio visual (para que se distinga a simple vista de `docs.*`, que es relativa a `workFolder` mismo) — es opcional y todo script `pv-*` la quita antes de resolver la ruta, así que `"previo-sdd"` y `"/previo-sdd"` funcionan igual; nunca es una ruta absoluta real del sistema de ficheros. Es el único campo de `framework` que `pv-init` nunca pregunta ni confirma: siempre escribe el default en silencio, igual que `skills.mockups`/`skills.diagrams`. Si algún día se quiere otra carpeta, se cambia a mano en `pv-context.json`, bajo la responsabilidad de quien lo edite. Se resuelve vía `resolve-path.py` — ver "Resolución de rutas" más abajo. Dentro de ella, `scaffold-project.py` de `pv-init` crea tres subcarpetas de nombre fijo justo después de escribir `pv-context.json`, que el usuario no elige ni renombra:
  ...
  - `{workFolder}/stuff/` — ficheros propios del proyecto que ninguna otra skill del framework decide por él: `how-to-compile.md` (procedimiento de build que `pv-version` pregunta y escribe la primera vez que hace falta) y `hooks/` (una subcarpeta por cada skill que expone hooks; `hooks/version/` contiene los cuatro ficheros de hook de `pv-version`, leídos en su paso 0.6 — `05-before-guardrail` ejecutado en el paso 0.4; `hooks/do/` contiene los dos de `pv-do` — `10-before-implementation.md`, `20-after-implementation.md` — leídos en su paso 1.5, y también por el subflujo rápido de `pv-fix` (que edita código directamente); `hooks/how/` contiene los dos de `pv-how` — `10-before-analysis.md`, `20-after-plan.md` — leídos en su paso 1.2; `hooks/new/` contiene el de `pv-new` — `20-after-entry.md` — leído en su paso 0.3. `scaffold-project.py` siembra cada conjunto vacío desde el arranque).
...
**Convención: todo `SKILL.md` `pv-*`/`pv-internal-*` declara su propia regla de idioma al principio.** Justo después del párrafo introductorio de la skill (la prosa bajo el H1 `# nombre-skill`) — antes de cualquier otro párrafo en negrita de alcance/comportamiento, y antes del primer encabezado `##` — va un párrafo **`**Language.**`** que indica exactamente qué campo(s) `framework.*.language` gobiernan lo que esa skill escribe/dice, con su default y cadena de fallback, y señalando cualquier excepción de idioma fijo (`docs.tech`, `stuff/hooks/*/*.md`, los encabezados marcados `[[[...]]]` de una plantilla). Una skill que no escribe nada y nunca habla con el usuario también lo declara explícitamente aquí (p. ej. `pv-internal-doc-files`, `pv-internal-tech-risks`), en vez de omitir el párrafo — así nadie tiene que inferir el comportamiento de idioma a partir de menciones dispersas más abajo en el fichero. Es una convención de redacción de `SKILL.md`, no un campo del schema de `pv-context.json`: nada la aplica automáticamente (ni siquiera la auditoría de `pv-update`), así que depende de quien escriba o revise una skill `pv-*` colocarla ahí.
...
Comprueba también los ficheros semilla de hook de cada skill que expone hooks bajo `{workFolder}/stuff/hooks/<subdir>/` (`version` → `pv-version`, `do` → `pv-do`, `how` → `pv-how`, `new` → `pv-new`): una semilla que falte (`stuff-<subdir>-hook-missing:<NN>`, `optional`) se recrea re-ejecutando `scaffold-project.py` (copia el `<NN>-<slug>.template.md` de la skill dueña, encabezado + cero pasos, sin sobrescribir); un fichero con el id `<NN>` correcto pero el slug equivocado (`stuff-<subdir>-hook-badslug:<file>`) se renombra al nombre canónico — es también la vía de migración cuando el framework renombra un punto de inserción (p. ej. `do/10-before-start` → `10-before-implementation`, los slugs `pre-`/`post-` de `version/` → `before-`/`after-`), conservando el contenido para que los pasos del proyecto sobrevivan. Solo `version` tiene además una forma de pipeline de fichero único pre-hooks (`{workFolder}/stuff/custom-version-pipeline.md`, o el intermedio `stuff/hooks/custom-version-pipeline.md`), tratada según cuántos pasos tenga: sin ningún `### Step` bajo ninguna sección es una semilla intacta (`stuff-pipeline-legacy-obsolete`) — se borra automáticamente y `scaffold-project.py` resiembra la nueva estructura; con pasos reales (`stuff-pipeline-legacy-location`) `pv-update` **no** lo migra — el cuarto caso no auto-corregible —, reporta el mapeo `## sección → NN-slug.md` para que la persona usuaria mueva los bloques. Mientras ese fichero antiguo está presente, las comprobaciones de semilla de `version` se saltan esa pasada (otros conjuntos como `do` sí se ejecutan). El fichero antiguo se lee solo para contar encabezados `### Step`; el contenido de los ficheros de hook por punto no se inspecciona nunca.

## Diagramas de flujo (workflow diagrams)

Algunas skills `pv-*` tienen un flujo con varios pasos y ramas (`pv-init`, `pv-update`, `pv-fix`, `pv-how`, `pv-new`, `pv-do`, `pv-version`). Para esas, la secuencia y las ramas viven en un fichero Mermaid dedicado junto al `SKILL.md` de la skill, y la skill lo lee **primero**, antes de ejecutar cualquier paso, siguiéndolo como fuente de verdad en vez de improvisar el flujo solo a partir de la prosa.

**Nodos de hook**: una skill con puntos de inserción de hooks de proyecto (`pv-version`, `pv-do`, `pv-how`, `pv-new` — los ficheros de hook bajo `{workFolder}/stuff/hooks/<subdir>/`; más `pv-fix`, cuyo subflujo rápido reutiliza el conjunto `stuff/hooks/do/` de `pv-do`) dibuja cada punto de hook en naranja mediante `classDef hook fill:#d9770e,color:#fff`, aplicado tanto al nodo de decisión "¿este hook define pasos?" como al nodo "ejecutar esos pasos", y añade una línea de leyenda explicando el color. Es el mismo naranja que se usa para el punto de extensión en el `version-flow-diagram.template.md` orientado al usuario de `pv-version`.
...
**Nombre y ubicación del fichero**: `workflow.<nombre-flujo>.md`, dentro de la propia carpeta de la skill (p. ej. `pv-init/workflow.init.md`, `pv-update/workflow.audit.md`, `pv-update/workflow.install.md`, `pv-fix/workflow.fix.md`, `pv-how/workflow.how.md`, `pv-new/workflow.new.md`, `pv-do/workflow.do.md`, `pv-version/workflow.version.md`) — no hace falta repetir el nombre de la skill en el fichero, ya lo indica la carpeta que lo contiene. Una skill puede tener más de un fichero de este tipo si su `SKILL.md` cubre varias entradas/flujos independientes (`pv-update` es el ejemplo: `workflow.audit.md` para el modo audit por defecto, `workflow.install.md` para el modo install explícito).
...
- `<NN>` — dos dígitos, orden numérico = orden de ejecución dentro de la skill. Múltiplos de `10` para los puntos "principales"; los huecos (`00`–`09`, `11`–`19`, …) quedan reservados para intercalar puntos futuros sin renumerar (`version/05` estrena el `05`). El `<NN>` **no** codifica `before`/`after` — eso lo dice la partícula temporal.
...
| `pv-version` | `version/10-before-version` | paso 0.7, tras el guardarraíl, antes de resolver `XXXX` |
...
│       ├── pv-new/                     # documenta un cambio intencional
│       │   └── hooks/                        # semilla NN-slug.template.md copiada a stuff/hooks/new/
...
│       │   ├── how-to-compile.template.md   # plantilla que pv-version copia al escribir stuff/how-to-compile.md
│       │   └── hooks/                        # semillas NN-slug.template.md que scaffold-project.py (pv-init) copia a stuff/hooks/version/
...
└── previo-sdd/                        # {workFolder} — default fijo, no se pregunta
    ...
    │   ├── how-to-compile.md       # procedimiento de build, escrito por pv-version (creación perezosa)
    │   └── hooks/                          # nombre fijo — una subcarpeta por proceso con hooks
    │       ├── version/                    # ficheros de hook de pv-version, sembrados vacíos por scaffold-project.py
    │       │   ├── 05-before-guardrail.md  # corre antes del guardarraíl de implemented/ (XXXX aún sin resolver)
    │       │   ├── 10-before-version.md    # corre tras el guardarraíl, antes del paso 1 (XXXX aún sin resolver)
    │       │   ├── 20-after-build.md       # corre tras dejar los artefactos en versions/XXXX/files/
    │       │   └── 30-after-changelog.md   # corre tras el changelog, antes del resumen
    │       ├── do/                         # ficheros de hook de pv-do (también los usa la vía rápida de pv-fix)
    │       │   ├── 10-before-implementation.md  # corre antes de editar código
    │       │   └── 20-after-implementation.md     # corre tras código + docs, antes de mover la carpeta
...
- `stuff/how-to-compile.md` es de creación perezosa: no existe hasta que `pv-version` lo necesita por primera vez (o hasta que se reporta un cambio en el procedimiento de build sin pedir entrega). Los ficheros `stuff/hooks/version/*.md`, en cambio, los siembra `scaffold-project.py` desde el arranque (encabezado + cero pasos, para que el mecanismo sea descubrible); `pv-update` recrea cualquiera que falte en un proyecto antiguo. Ninguno se sobrescribe una vez tiene contenido.
