...
- **pv-update** — Audita y repara `.claude/pv-context.json` y todo lo que configura contra el estado real del repo (forma del esquema, claves obsoletas, skills referenciadas, rutas en disco, la semilla de namespace de `architectureDocDir`, las semillas `stuff/hooks/*/*.md`, la frescura de `pv.py`, la deriva de `skillModels`, las etiquetas estructurales marcadas con `[[[...]]]`, códigos de change duplicados, y la consistencia de versión entre todas las skills `pv-*`) — arregla todo lo que puede determinar sin ambigüedad y reporta qué cambió; solo pregunta al usuario cuando `pv-context.json` es JSON roto, la versión instalada parece un downgrade, o el fichero de un ancla de namespace ya no existe. También es el propio mecanismo de instalación/actualización del framework (`/pv-update install`): resuelve una versión objetivo (última release oficial, o un `--version` explícito), rechaza un downgrade o un tag no parseable sin flag de escape, y — solo si la comprobación pasa — delega la descarga/sincronización real en `install.sh`/`install.ps1` de la raíz del repo. Este segundo modo nunca se dispara por sí solo; el flujo por defecto, solo auditoría, nunca toca la red ni instala nada. El paso 0 de cualquier otra skill pública `pv-*` ejecuta `check-framework-status.py` (más abajo) como comprobación barata antes de hacer su propio trabajo, y envía al usuario aquí si falla. *Usa:* ninguna otra skill.
  ...
  - [`workflow.install.md`](skills/pv-update/workflow.install.md) — diagrama Mermaid del flujo del modo instalación (ver "Diagramas de flujo" arriba): consultar GitHub Releases para la última versión oficial y cualquier pre-release más reciente, resolver la versión objetivo, rechazar un downgrade o tag no parseable, avisar (sin bloquear) si el objetivo es una pre-release, invocar `install-framework.py`, reportar y recordar al usuario correr `/pv-update` (modo audit) a continuación.
  - [`scripts/install-framework.py`](skills/pv-update/scripts/install-framework.py) — detecta el SO (`platform.system()`, nunca por qué binarios hay en `PATH`) e invoca `install.ps1` (Windows) o `install.sh` (resto) en la raíz del repo con el tag resuelto. Consulta `GET /repos/{REPO}/releases/latest` y `GET /repos/{REPO}/releases?per_page=1` para informar siempre de la última release oficial y, si es más reciente, una pre-release, antes de decidir nada. Rechaza de forma dura, sin flag de escape, un tag `--version` no parseable o un downgrade real contra la versión instalada — nunca delega esa comparación en `install.sh`/`.ps1`. Nunca ejecuta `mark-verified.py` por sí misma.
