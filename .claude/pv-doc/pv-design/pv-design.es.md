...
- **pv-version** — Prepara una entrega en `{workFolder}/versions/{XXXX}/`: exige primero que `{changesDir}/implemented/` esté vacío (cada entrada se resuelve moviéndola a `closed`), genera el entregable siguiendo `{workFolder}/stuff/how-to-compile.md` (procedimiento propio del proyecto, escrito la primera vez que hace falta, capaz de describir varios pasos si el build genera varios artefactos), comprime en `.zip` y copia `docs.tech.architectureDocDir`/`docs.tech.styleBibleDocDir`/`docs.functional.featuresDocPathDir` que estén configuradas, y encadena `pv-internal-changelog` para el changelog funcional. Si se invoca solo para informar de un cambio en el procedimiento de build, actualiza `{workFolder}/stuff/how-to-compile.md` sin lanzar el resto del proceso salvo confirmación explícita. `{XXXX}` es texto libre elegido por el usuario en cada invocación, sin relación con la numeración `xxxx` de change/fix ni con ninguna otra carpeta "versions" que exista en el repo. *Usa:* `pv-internal-changelog`.
  ...
  **Puntos de personalización (no se edita la skill).** El `SKILL.md`, `workflow.version.md` y demás ficheros bajo `.claude/skills/pv-*/` son framework instalado, gestionado vía `pv-update`; editarlos a mano los deja inconsistentes. Hay exactamente dos puntos de personalización en `{workFolder}/stuff/`: cómo compilar el entregable → `how-to-compile.md` (pasos 3–4); los pasos propios del proyecto en tres momentos del flujo → los ficheros de hook bajo `hooks/version/` (paso 0.6), uno por punto: `10-pre-release.md` (paso 0.7, antes del paso 1, solo `{workFolder}` sustituido; `{XXXX}` aún no resuelto), `20-post-build.md` (paso 4.1, tras copiar los artefactos a `files/`; `{XXXX}` y las rutas `versions/{XXXX}/` disponibles) y `30-post-changelog.md` (paso 6.1, tras el changelog y antes del resumen; mismas variables). Cada uno con 0..N bloques `### Step N: {name}` de la misma forma que `how-to-compile.md`; `pv-version` casa un fichero por su prefijo de id `NN-` (el `slug` también es fijo). Un fichero sin pasos, o ausente, se salta en silencio; si un paso falla, el flujo se detiene y se explica (mismo criterio que el paso 4). El resumen final (paso 7) indica qué hooks se ejecutaron y qué produjeron. Si lo pedido por el usuario no encaja en ninguno de los puntos, se le dice y se propone abrir un cambio en el repo del framework — nunca un parche local a la skill.
  ...
  - [`hooks/*.template.md`](skills/pv-version/hooks/) — semillas de `{workFolder}/stuff/hooks/version/<NN>-<slug>.md`, una por punto de inserción (`10-pre-release`, `20-post-build`, `30-post-changelog`): un encabezado que explica cuándo corre el hook y qué variables están disponibles, más un esqueleto `### Step N` comentado, cero pasos vivos. Las copia `scaffold-project.py` de `pv-init` (no `pv-version`), cada una solo si falta; `pv-version` (paso 0.6) casa un fichero por su prefijo de id `<NN>` y ejecuta los bloques `### Step N` que contenga. Nunca se sobrescribe una vez tiene contenido.
  ...
  - [`version-flow-diagram.template.md`](skills/pv-version/version-flow-diagram.template.md) — diagrama Mermaid general y simplificado del proceso de `pv-version` (guardarraíl de `implemented/` vacío → pasos de hook propios del proyecto (`stuff/hooks/version/*.md`) → crear carpeta → compilar → comprimir docs → changelog → confirmar), pensado para mostrarse tal cual si el usuario pregunta cómo funciona `/pv-version` — solo la información relevante y comprensible para el usuario, no todo el ramificado interno que sí cubre `workflow.version.md`. El nodo de hook va en naranja (`classDef extension`), distinto de los pasos mecánicos azules.
...
- **`workFolder`** (`string`, opcional, default `"/previo-sdd"`): carpeta relativa a la raíz del repo bajo la que el framework gestiona todo su trabajo. La `/` inicial es solo un convenio visual (para que se distinga a simple vista de `docs.*`, que es relativa a `workFolder` mismo) — es opcional y todo script `pv-*` la quita antes de resolver la ruta, así que `"previo-sdd"` y `"/previo-sdd"` funcionan igual; nunca es una ruta absoluta real del sistema de ficheros. Es el único campo de `framework` que `pv-init` nunca pregunta ni confirma: siempre escribe el default en silencio, igual que `skills.mockups`/`skills.diagrams`. Si algún día se quiere otra carpeta, se cambia a mano en `pv-context.json`, bajo la responsabilidad de quien lo edite. Se resuelve vía `resolve-path.py` — ver "Resolución de rutas" más abajo. Dentro de ella, `scaffold-project.py` de `pv-init` crea tres subcarpetas de nombre fijo justo después de escribir `pv-context.json`, que el usuario no elige ni renombra:
  ...
  - `{workFolder}/stuff/` — ficheros propios del proyecto que ninguna otra skill del framework decide por él: `how-to-compile.md` (procedimiento de build que `pv-version` pregunta y escribe la primera vez que hace falta) y `hooks/` (una subcarpeta por cada skill que expone hooks; `hooks/version/` contiene los tres ficheros de hook de `pv-version`, leídos en su paso 0.6; `hooks/do/` contiene los dos de `pv-do` — `10-before-start.md`, `20-before-finish.md` — leídos en su paso 1.5, y también por el subflujo rápido de `pv-fix` (que edita código directamente). `scaffold-project.py` siembra cada conjunto vacío desde el arranque).
...
**Nodos de hook**: una skill con puntos de inserción de hooks de proyecto (`pv-version`, `pv-do` — los ficheros de hook bajo `{workFolder}/stuff/hooks/<subdir>/`; más `pv-fix`, cuyo subflujo rápido reutiliza el conjunto `stuff/hooks/do/` de `pv-do`) dibuja cada punto de hook en naranja mediante `classDef hook fill:#d9770e,color:#fff`, aplicado tanto al nodo de decisión "¿este hook define pasos?" como al nodo "ejecutar esos pasos", y añade una línea de leyenda explicando el color. Es el mismo naranja que se usa para el punto de extensión en el `version-flow-diagram.template.md` orientado al usuario de `pv-version`.
...
└── previo-sdd/                        # {workFolder} — default fijo, no se pregunta
    ...
    │           ├── 10-pre-release.md       # corre antes del paso 1 (XXXX aún sin resolver)
    │           ├── 20-post-build.md        # corre tras dejar los artefactos en versions/XXXX/files/
    │           └── 30-post-changelog.md    # corre tras el changelog, antes del resumen
    │
