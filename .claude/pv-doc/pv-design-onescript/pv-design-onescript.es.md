...
└── "Previo v{version}: MAIN MENU" ({version} = metadata.version de pv-init/SKILL.md)  [Menu]
    ...
    │       ├── [3] Acción: Install new Previo version (cualquier tag ANTERIOR al instalado se excluye del todo de la lista — ver más abajo; si eso vacía la lista, un Info dice que Previo ya está al día y vuelve, sin mostrar listado)
    │       │   └── Listado "Allowed Previo versions:" (propio, no show_selection() — ver más abajo) + bucle
    │       │       ├── Input: un número listado → ¿tag == instalado? imprime aviso de "reinstalación desde cero"; si no, sin texto extra — luego → Confirmation: "Install Previo {tag}?"  [Confirmation]
...
graph TD
    ...
    N -->|Search by state| P
    P -->|Return| N
    N -->|Toggle a flag on a change| TF["🏷️ Toggle flag<br/>Listado por id (bucle) + Selección incrustada (bucle)<br/>+ set-metadata.py (sin confirmación)"]
    TF -->|Return| N
...
Segundo caso, por un motivo distinto: el listado "Allowed Previo versions:" de `install_previo_version()` **sí** va numerado por fila, pero también evita `show_selection()` — ese helper devuelve `None` tanto para "input vacío" como para "escribió algo inválido", sin forma de que el caller distinga uno de otro, y esta pantalla necesita que ambos casos se comporten distinto ("volver atrás" vs. "Invalid option., quedarse en la misma pantalla"). Así que renderiza el mismo marco `hr("-")` + lista numerada + `hr("-")` que usaría `show_selection()`, pero lee la elección con `read_input()` y distingue él mismo vacío de fuera-de-rango.

**Los tags anteriores al instalado se filtran, no se ofrecen con aviso.** `install-framework.py` rechaza de forma dura instalar un tag anterior a la versión instalada sin excepción — ni siquiera con `--yes`, sin override alguno (ver "Dependencias externas") — así que `install_previo_version()` nunca lo lista en primer lugar: listarlo y dejar que el usuario confirme solo garantizaría un fallo posterior. Antes de construir `options`/`tags`, tanto `official_tag` como `prerelease_tag` se comparan contra `framework_version()` (la versión instalada) vía `_semver_tuple()` — un pequeño parser `(major, minor, patch)` duplicado del propio `parse_version()` de `install-framework.py`, mantenido sincronizado por convención, el mismo patrón de script autocontenido que sigue el resto de este fichero — y cualquier tag estrictamente anterior al instalado se descarta. Este es el caso en que hay una pre-release instalada y la última release "oficial" es anterior a ella: solo sobrevive al filtro el tag de la pre-release (igual al instalado), nunca el oficial más viejo. **Si todos los tags conocidos quedan filtrados**, la lista quedaría vacía — en vez de mostrar un listado "Allowed Previo versions:" vacío, `install_previo_version()` muestra un Info (`framed=False`) diciendo que Previo ya está al día y vuelve directamente a "Previo: settings", sin entrar en el bucle. **El único caso restante, no bloqueante:** elegir el **tag exactamente instalado** (sigue permitido, porque no es anterior, solo igual) imprime una línea de aviso plana (sin color) justo antes del prompt de `confirm()` — continuando el mismo bloque sin DARK_GRAY, sin `hr()` propio, igual que cualquier texto justo antes de una Confirmation — de que esto reinstalará todo desde cero. No bloquea la elección ni cambia el texto de `confirm()`, es solo contexto informativo leído justo antes de responder la misma pregunta `"Install Previo {tag}? This modifies files in this project."` que recibe cualquier otra elección.
...
5. Si el listado de objetivos es largo o heterogéneo, agrúpalo con encabezados como hace `toggle_flag_on_change()` — que reusa la misma partición que "General project status" (🟢 `implemented/*`, 🟠 `inProgress/*` con `plan.md`, 🟡 `inProgress/*` sin `plan.md`; `closed/*` se deja fuera por congelado). `show_selection()` no sabe pintar encabezados de grupo: imprime el listado tú mismo y, como el código de cada entrada ya es un identificador estable, muestra las filas **sin numerar** (`{código} — {nombre}`) y lee la elección con `read_input()`, resolviendo el id tecleado contra las entradas con un matcher tipo `_ids_match()` (padding de ceros ignorado) — no una numeración de fila paralela. Los encabezados de grupo van en **GOLD** (excepción puntual a "un color por pantalla", igual que los colores por estado de `search_by_state()`) para separarlos de las filas de entrada en DARK_GRAY.
