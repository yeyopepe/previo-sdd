...
- **2026-09-24 (tercera pasada, durante verificación Fase 4)**: petición explícita del usuario tras probar el asset en vivo — una nota `closed` solo ofrecía 🗑 Eliminar, sin forma de deshacer un cierre mal resuelto (el cambio aplicado no era el correcto, o el reviewer quiere añadir más contexto). **Nueva capacidad del framework de anotaciones (navegador)**: un botón **↺ Reopen** en toda nota `closed` (tarjeta y panel), que la devuelve a `state: "open"` conservando su texto. Esto no contradice la regla existente de que "el navegador nunca escribe `closed`" — esa transición (`open→closed`) sigue siendo exclusiva de `ensure-closed`; `Reopen` es la dirección inversa (`closed→open`), y el navegador ya podía libremente crear notas en `open` desde el principio, así que no se le concede ningún poder nuevo sobre el ciclo de vida que no tuviera ya. El botón 🗑 Eliminar en notas `closed` se mantiene sin cambios (decisión explícita: el usuario prefiere que una nota resuelta persista como registro visible salvo borrado manual, aunque reconoce que en la práctica probablemente nunca las borre). Implementado en `assets/mockup-annotations.html` (`reopenNote()`, botón en `renderCardFoot`/`renderPanelItemActs`).
