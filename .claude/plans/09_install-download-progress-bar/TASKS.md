...
- [x] Sustituir `curl -fsSL "$TARBALL" | tar -xz -C "$TMP" --strip-components=1`
...
- [x] Declarar `TAR_PATH="$TMP/previo.tar.gz"` antes de la descarga.
- [x] Envolver la llamada a curl con `printf '\033[34m'` antes y
      `printf '\033[0m'` después, para que la barra se imprima en azul.

## 2. `install.ps1` — barra de progreso

- [x] Sustituir `Invoke-WebRequest -Uri $Tarball -OutFile $TarPath`
      ...
      con `` `r ``).
- [x] Añadir `Add-Type -AssemblyName System.Net.Http` antes del bloque de
      descarga.
- [x] Envolver la descarga en `try/finally` con `$httpClient.Dispose()`,
      dejando intacto el `try/finally` externo existente que limpia `$Tmp`.
- [x] En `Write-ProgressBar`, imprimir el relleno `$bar` (`=`/`>`) con
...
- [x] Sustituir el bloque de resolución de `TAG` (líneas 15-23) para que,
...
- [x] Añadir la variable `INSTALLED_FROM_RAW_TAG` (0/1) para saber, más
      adelante en el script, si hay que mostrar el aviso de tag no
      publicado.
- [x] Si el tag puro existe, usar `TAG="$REQUESTED_TAG"` directamente
      (sin pasar por `RELEASE_JSON`) y continuar el flujo normal
      (`TARBALL`, descarga, extracción) sin cambios.
- [x] Si ni el release ni el tag puro existen, mantener el mensaje de
      error actual ("Version '...' doesn't exist in Previo's releases.")
      y el `exit 1`.
- [x] Añadir, junto a los otros avisos de bloque `====` (tras la sección
...
- [x] Aplicar el mismo tratamiento (envolver con `printf '\033[33m'` /
...
- [x] Sustituir el bloque de resolución de `$Tag` (líneas 15-26) para que,
...
- [x] Añadir la variable `$InstalledFromRawTag` (bool) para saber, más
      adelante en el script, si hay que mostrar el aviso de tag no
      publicado.
- [x] Si el tag puro existe, usar `$Tag = $Version` directamente y
      continuar el flujo normal (`$Tarball`, descarga, extracción) sin
      cambios.
- [x] Si ni el release ni el tag puro existen, mantener el mensaje de
      error actual (`throw "Version '$Version' doesn't exist in Previo's
      releases."`).
- [x] Añadir, junto a los otros avisos de bloque `====` (junto al aviso
      de changelog faltante), el nuevo bloque de aviso cuando
      `$InstalledFromRawTag` sea `$true`, con cada línea usando
      `-ForegroundColor Yellow`.
- [x] Aplicar `-ForegroundColor Yellow` al bloque de aviso ya existente
