...
## Post-mortem: por qué los intentos de implementación (v0.9.8b1→b4) fracasaron

Este plan se marcó como implementado y se borró tras el primer intento (commit
`56226fa`, "Added: todo progress tracking"), pero el sistema resultante nunca llegó a
funcionar de forma fiable a través de varias rondas de "fix" (`f5363a7`, `d7e2e32`,
`4009f3e`). Se ha revertido por completo — código, docs y skill — y este plan se ha
restaurado tal cual estaba antes de darse por bueno. Motivos concretos observados:

1. **El plan decía "la instrumentación va en el diagrama, no en prosa nueva del
   `SKILL.md`" — y no se siguió a la primera.** El primer intento (`56226fa`) sí
   instrumentó `workflow.do.md`/`workflow.fix.md`/`workflow.how.md`/`workflow.new.md`
   correctamente. Pero al arreglar temas relacionados en la misma sesión (p. ej. añadir
   `pv-update` al sistema, que no estaba en el alcance original de este plan) se coló
   prosa nueva y redundante en varios `SKILL.md` — párrafos enteros de "Progress
   checklist" con la lista de items repetida, cuando esa lista ya vivía en el diagrama.
   Esto violaba la regla explícita de la sección "Puntos de invocación" de este mismo
   documento, y generaba dos fuentes de verdad que podían (y empezaron a) divergir.

2. **`framework.skills.progress` se implementó con `default` en al menos un intento,
   contradiciendo el diseño "opt-in puro, sin default" de este plan.** En un momento
   dado `schema.json` y `pv-init/SKILL.md` llegaron a escribir
   `"progress": "pv-internal-progress-todowrite"` automáticamente en todo proyecto
   nuevo, exactamente el comportamiento que la sección "Configuración" de este plan
   dice explícitamente que NO debía pasar (`mockups`/`diagrams` sí llevan default
   porque producen contenido necesario; `progress` es cosmético y por diseño solo se
   activa si el usuario lo pide). Esto obligó a un commit de "fix" posterior
   (`f5363a7`) solo para deshacer el default, y ese fix a su vez introdujo un nuevo
   check (`skills-progress-unconfigured`) en `pv-update` que tampoco estaba en el
   diseño original — una quinta excepción "pregunta al usuario" que no aparecía en
   ningún sitio de este plan.

3. **Cada "fix" fue un parche reactivo sobre el problema más visible del momento, sin
   volver a leer este plan como fuente de verdad.** El patrón repetido en los tres
   commits de corrección fue: algo no funcionaba → arreglo puntual → bump de versión →
   siguiente síntoma descubierto en la siguiente sesión. Nunca se hizo una comparación
   sistemática contra este documento para verificar que el resultado seguía cumpliendo
   el diseño original (contrato `action`/`items`/`itemId`/`status`, opt-in sin default,
   instrumentación solo en el diagrama, granularidad de pasos mayores). Eso permitió que
   la implementación se desviara progresivamente del plan sin que nadie lo notara hasta
   que, tras cuatro intentos, seguía "sin funcionar" a ojos del usuario.

4. **No hubo una prueba de extremo a extremo real que hubiera detectado esto pronto.**
   La "Lista de tareas para la implementación" de este plan incluye un punto 11 de
   prueba manual explícito (activar `skills.progress`, correr un `pv-fix` no-trivial y
   uno fast-track, comprobar que la checklist aparece y se cierra bien; repetirlo sin
   el campo configurado). No hay evidencia de que ese paso se ejecutara en ninguno de
   los cuatro intentos — de haberse hecho, la contradicción del punto 2 (default
   apareciendo en un proyecto nuevo sin haberlo pedido) se habría visto en el primer
   intento, no cuatro commits después.

**Lección para el próximo intento:** antes de dar por buena cualquier reimplementación,
releer este plan completo y verificar explícitamente cada punto de "Decisiones ya
confirmadas en el diseño" contra el resultado real (diff de `schema.json`, de cada
`SKILL.md`, y de cada `workflow.*.md`) — no solo comprobar que "algo aparece en la
interfaz", sino que el mecanismo de activación, el contrato y el lugar donde vive la
instrumentación coinciden exactamente con lo aquí descrito.
