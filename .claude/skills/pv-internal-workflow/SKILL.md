...
**Language.** Use `framework.interaction.language` (default English) for the guardrail message to the user in the section below. `description.md`/`history.md` (action `create`) follow `framework.changes.language` (default `interaction.language`, English if neither is configured) — except everything wrapped in `[[[...]]]` in `description.template.md`, which stays fixed in English always (see the "Marker convention in templates" section of `pv-design.en.md`): that's the four field labels (**Name**, **Code**, **Type**, **Creation date**) *and* the `## Full description` section heading. **`[[[...]]]` is template-only syntax, never valid output**: when writing the real `description.md`, drop the triple brackets entirely and keep only the label inside them, unchanged, in English — `**[[[Name]]]**:` becomes `**Name**:`, `## [[[Full description]]]` becomes `## Full description`. The brackets must never appear in the generated file, translated or not. The `## Technical notes` heading is **not** marked (it's optional — omitted entirely when there's nothing technical to note, which a marker check can't tell apart from a translation); still write it in English for consistency, but it carries no marker. `pv-status`'s `filter_status.py` parses `## Full description` literally (`extract_description`) for the status report; translating that heading, or leaving the brackets in, makes the entry's summary show up empty there, silently. If `language` is not configured anywhere, everything is English.
