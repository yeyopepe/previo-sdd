...
3. **Generate the visual proposal and the navigation diagram.** If the change has a visual component (there's something to say in step 1's "High-level visual definition" point):
   - **HTML mockups.** Invoke (Skill tool) the mockups skill configured in `.claude/pv-context.json`'s `framework.skills.mockups` (if not configured, `pv-internal-mockups-html`), passing it the destination folder (`{changesDir}/inProgress/{xxxx}/mockups/`) and, for each distinct visual element in the proposal, its description and what it should show (e.g. one element for the deck-selection modal, another for the progress bar), marking the action as `create`. Only invoke it when there's actually at least one element to mock up — never "just in case". Note the `design_*.html` paths it returns. If it also returns a non-empty **style gaps found** list (styling it had to pull from the real code because `docs.tech.styleBibleDocDir` didn't cover it), don't drop it silently: add it to **Technical notes** in `description.md` (step 2) as a pending task, not just an observation — e.g. "Technical notes: `docs.tech.styleBibleDocDir` doesn't cover `<gap>`; `<value/convention>` was pulled from `<code location>` instead — update the style bible with this when implementing." so `pv-how`/`pv-do` pick it up as something to actually do, not just read.
...
4. **Validate the visual representation with the user.** If step 2 included a Mermaid diagram, or steps 3/3.1 generated any `design_*.html`, `navigation_*.md` or `data_*.md`, don't consider them final just because they were written: present them to the user (point them to each `design_*.html`/`navigation_*.md`/`data_*.md`'s path so they can open or review it, and show the Mermaid diagrams) and ask them to confirm whether they reflect what they had in mind or what they'd change.
