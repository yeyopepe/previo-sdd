...
- **UI navigation or interaction** (a screen change, a modal or dropdown opening, or any visual state transition triggered by a user action, even if it doesn't leave a single screen): a navigation diagram (`navigation_*.md`, step 3).
- **Data with its own structure** (the change defines or uses something that needs a list of properties or associated data — an object's properties, a table's content, a configuration's fields, etc.): data table(s) (`data_*.md`, step 3.1).
...
1. **Understand the scope and anticipate the usual functional doubts.** Don't wait for an obvious ambiguity to come up: before documenting, review the request and the relevant project code to build your own list of points that usually stay undefined in this kind of change. Go over at least:
   ...
   - **Coexistence with what exists**: whether this replaces, complements, or conflicts with functionality already present in the project. While reviewing other entries under `{changesDir}/**` for this, also note whether this change looks related to any existing change/fix (`xxxx`) — either because the user mentioned it explicitly, or because your own review found a clear connection. Include any such id(s) in the list you present the user to confirm ("this looks related to change 00042 — is that right?"); don't invent a relation that isn't reasonably supported.
...
2. **Document the intent.** Invoke the `pv-internal-workflow` skill (Skill tool) with `action=create`, `type=change`, the functional summary of what's being asked — including step 1's list of doubts already resolved (confirmed proposals, the user's corrections and, if applicable, the agreed high-level visual definition) — and `promptOriginal` (the request exactly as the user wrote it, without rephrasing), so it can number the change and create `description.md` and `history.md` at `{changesDir}/inProgress/{xxxx}/`. Note the `xxxx` it returns: you need it in the next step.
   ...
   If step 1 confirmed one or more related change/fix ids, record them right after, via `set-metadata.py`:
   ```
   python .claude/skills/pv-internal-workflow/scripts/set-metadata.py --xxxx {xxxx} --add-related {otherXxxx} [--add-related {otherXxxx2} ...]
   ```
   Skip this call entirely if no relation was confirmed — don't create an empty `.metadata.json` just for this.
...
3. **Generate the visual proposal and the navigation diagram.** If the change has a visual component (there's something to say in step 1's "High-level visual definition" point):
   - **HTML mockups.** Invoke (Skill tool) the mockups skill configured in `.claude/pv-context.json`'s `framework.skills.mockups` (if not configured, `pv-internal-mockups-html`), passing it the destination folder (`{changesDir}/inProgress/{xxxx}/mockups/`), the action `create`, and, for each distinct visual element in the proposal, its description and what it should show (e.g. one element for the deck-selection modal, another for the progress bar). Also pass `style_context` — any style-bible excerpts (tokens/colors, typography, spacing, conventions) already gathered from step 1's `pv-internal-tech-analysis` call that are relevant to these elements; if nothing style-relevant came up, pass nothing, the mockups skill already defaults to neutral styling on its own. Only invoke it when there's actually at least one element to mock up — never "just in case". Note the `design_*.html` paths it returns.
...
5. **State the next step.**
   ...
   **Hook: `20-after-entry`.** First, if `20-after-entry.md` (step 0.3) defines `### Step` blocks, run them **now**, in order — after step 4 validated the `design_*` and after the `todo/` idea (if this was `/pv-new todo <code>`) is already deleted, with the entry in `{changesDir}/inProgress/{xxxx}/`. `{workFolder}` and `{xxxx}` are substituted. Run each step's command(s) from the repo root and verify its output. If a step fails, **stop and explain it** — don't improvise an alternative (the entry is already documented on disk; only the external registration is missing). No steps, or file absent: continue silently.
   ...
   Then tell the user the change is documented (`description.md`) and, if applicable, with its visual and data proposal already validated (`design_*.html`, `navigation_*.md`, `data_*.md`); to plan and implement it they should invoke the `pv-how` skill on that `xxxx`. If the user wants to implement it right away, you can invoke `pv-how` directly yourself.

Don't write the change document yourself nor compute the `xxxx` number — `pv-internal-workflow` does that, to keep a single place with that logic. `design_*.html` files are generated by the configured mockups skill (`pv-internal-mockups-html` by default) — don't write them yourself, and don't resolve pending annotations on them yourself either: that's done exclusively via the mockups skill's `ensure-closed` action (step 4), which is also the only way you learn what changed — never read or parse a `design_*.html`'s own annotation data. Step 2's Mermaid diagram code is generated by the configured diagrams skill (`pv-internal-tech-mermaid` by default) — don't draft it yourself either. `navigation_*.md` and `data_*.md` files, however, you do write directly yourself: they're not the responsibility of any internal skill, which are project-agnostic and don't analyze or design anything.
