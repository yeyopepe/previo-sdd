...
2. Read that `description.md` in full (`## Idea`, `## Code` and `## Notes` sections) and, if it exists, the content of its `mockups/` subfolder. This is the content to analyze and document — use it as if it were the user's request for the rest of the process, instead of waiting for a new description in chat. If the user also added extra context when invoking the skill, add it to the analysis.
