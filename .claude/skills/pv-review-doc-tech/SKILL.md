...
Use `framework.interaction.language` (default English) for everything you say to the user in this conversation. The documentation itself stays in whatever language it's already written in (`docs.tech` is always fixed technical English per its schema) — this skill never translates.
...
- **Never delete a fact.** If a fact looks redundant, wrong, or outdated, that's a content judgment outside this skill's scope — leave it and, if worth flagging, mention it in the final summary instead of acting on it.
...
- **Never touch `00-*.md` files' content** (e.g. `00-namespace.md`). They're owned by `pv-do` directly, not by the per-file `upsert` convention this skill works within. If reorganization would require changing a namespace path or anchor, stop and flag it to the user instead of editing it yourself.
...
Summarize, per folder: what was moved/consolidated and why (one line each), any folder skipped for being empty, and anything flagged in step 4 that needs a human decision (a namespace/anchor change, a fact that looked redundant but wasn't touched). Don't repeat the full diff — the edits are already visible in the working tree.
