...
**Language.** Use `framework.interaction.language` (default English) for everything you say to the user in this conversation. `plan.md` follows `framework.changes.language` (default `interaction.language`, English if neither is configured) for its prose — the body text of every section, the risk tables, the authoring notes — except everything wrapped in `[[[...]]]` in `PLAN.template.md`, which stays fixed in English always (see the "Marker convention in templates" section of `pv-design.en.md`): that's the header field (**Creation date**) and the three always-present section headings (`## (a) Functional notes`, `## (b) Technical solution`, `## (e) Verification`). `[[[...]]]` is template-only syntax, never valid output: when writing the real `plan.md`, drop the triple brackets entirely and keep only the label/heading inside them, unchanged, in English — `**[[[Creation date]]]**:` becomes `**Creation date**:`, `## [[[(a) Functional notes]]]` becomes `## (a) Functional notes`. The brackets must never appear in the generated file, translated or not. The conditional section headings — `## (c) Architecture changes`, `## (d) Style changes`, `## (f) Risk analysis` — are **not** marked (they're often omitted entirely, which a marker check can't tell apart from a translation); still write them in English for consistency, but they carry no marker. `pv-status`'s `filter_status.py` parses **Creation date** literally (`extract_date`, reused from `description.md`'s parsing) to build the status report; translating it makes the entry's planned date show up as missing there, silently. The section headings are kept in English so `pv-do`/`pv-how` (and `pv-update`'s marker check) can find them by name across languages. The one exception within the risk-factor prose is `pv-internal-tech-risks`' 9 risk factor names: it returns them in English as internal framework vocabulary (see the note in step 3.1) — translate them to `changes.language` yourself when writing section (f), don't leave them in English. When asking `pv-internal-tech-mermaid` for a diagram for `plan.md`, pass it `changes.language` as the target language. The hook files under `{workFolder}/stuff/hooks/how/`, which this skill reads and executes directly (steps 1.2, 3, 3.1.5), follow `interaction.language` — there is no dedicated language field for `stuff/*` in the schema. If `language` is not configured anywhere, everything is English.
...
**This skill is installed framework, not editable from a consumer project.** If the user asks to change *how* the analysis flow works (load some context before analyzing, run a linter or publish the plan/risk when it's written), the right answer is **not** to edit this `SKILL.md` or any file under `.claude/skills/pv-*/`. The customization point is `{workFolder}/stuff/hooks/how/*.md` — the project's own steps at two points of the flow (see step 1.2). If what's asked doesn't fit one of those two hooks, say so and propose opening a change in the framework repo — never a local patch to the skill. The presence of `framework.frameworkStatus` in `pv-context.json` means these skills are managed via `pv-update`; editing them by hand leaves them inconsistent (step 0 already checks versions).

**Before any other step**, read [`workflow.how.md`](workflow.how.md) — it's the source of truth for this flow's sequence and branches, including the two hook insertion points (see `pv-design.en.md`'s "Workflow diagrams" section for the notation). If it doesn't exist or can't be followed, stop and report that instead of improvising the flow from the prose below. The numbered steps that follow are each node's detail (which skill to invoke, what exact text to use) — the diagram governs sequence and branching; if the two ever disagree, the diagram wins and this prose gets corrected to match.
...
## 1.05 Gate: ensure mockups have no pending annotations

If `{changesDir}/inProgress/{xxxx}/mockups/` has any `design_*.html`, invoke the mockups skill configured in `.claude/pv-context.json`'s `framework.skills.mockups` (if not configured, `pv-internal-mockups-html`) with `action=ensure-closed`, passing every `design_*.html` in that folder. Wait for its OK before continuing — this is a hard precondition, not an optimization: don't assume `pv-new`/`pv-fix` already left it clean, even if they ran earlier in this same session. Don't read, describe, or otherwise use any `design_*.html` from this entry — including step 1.1's own document-consistency check — until `ensure-closed` returns.

This gate only covers `mockups/`: `navigation_*.md`/`data_*.md` live outside it, aren't owned by any mockups skill, and keep being read with a plain `Read` as usual (step 1.1). If the folder has no `design_*.html`, skip this step entirely.

## 1.1 Validate the change's documents before analyzing

Before gathering technical context or writing `plan.md`, look for inconsistencies or problems across `{changesDir}/inProgress/{xxxx}/`'s documents: `description.md`, the visual facts of anything under `mockups/` (obtained via the configured mockups skill's `action=describe` for the elements/areas relevant to this check — never `Read` a `design_*.html`/`design_*.txt` yourself), and `navigation_*.md`/`data_*.md` loose in the entry root if they exist (still read directly with `Read`) — two distinct locations, not one. Look for: requirements that contradict each other, contradictory information between `description.md` and the mockups/data tables, ambiguous steps or acceptance criteria, references to elements the mockups don't show (or vice versa), data `description.md` mentions but no `data_*.md` table records (or vice versa), gaps that prevent knowing what's being asked. No fallback to the legacy loose-in-root layout or the retired `design_navigation_*.md`/`design_data_*.md` names — an entry that hasn't been migrated yet is invisible here; `pv-update` is what guarantees no entry stays legacy.
...
- **If you find something**: don't resolve it on your own nor proceed with the analysis. Lay out the problem to the user clearly (which documents are involved and what the inconsistency or gap consists of) and ask them how to resolve it.
  ...
  With the user's answer, update the affected definition document(s) yourself (`description.md` and/or the mockup/`navigation_*.md`/`data_*.md` ones) so they're consistent before continuing. If the fix requires generating or editing a visual mockup, invoke (Skill tool) the skill configured in `.claude/pv-context.json`'s `framework.skills.mockups` (if not configured, `pv-internal-mockups-html`) instead of editing the mockup's HTML/ASCII yourself. Repeat this validation on the now-corrected documents before considering the step done.

## 1.2 Load the project's hooks

`pv-how`'s hooks live one file per insertion point at `{workFolder}/stuff/hooks/how/<NN>-<slug>.md` (`{workFolder}` is `framework.workFolder`, default `/previo-sdd`). Two points are defined:

| `<NN>` | file | runs |
|--------|------|------|
| `10` | `10-before-analysis.md` | start of step 3, before `pv-internal-tech-analysis` (not run when step 2 chooses "implement current `plan.md`") |
| `20` | `20-after-plan.md` | after step 3.1 (risk median persisted), before step 3.2 (ask whether to implement) |

List `{workFolder}/stuff/hooks/how/*.md`. For each file, take the `<NN>` id from its `^(\d+)-` prefix and read its `### Step N: {name}` blocks (`**Command(s) to run**` / `**Generated file(s)**` / `**Notes**`). A file that's absent, or present with zero `### Step` blocks, means its hook is skipped silently. A file whose `<NN>` matches no point above → ignore it, and mention it to the user (a typo, or a hook from a newer framework version). The `<slug>` after the id is fixed (`before-analysis` / `after-plan`); a file with the right id but a different slug still runs, but tell the user so `pv-update` can normalize the name.
...
Substitutable variables in the step commands: `{workFolder}` and `{xxxx}`, in both hooks (the entry already exists at `{changesDir}/inProgress/{xxxx}/`). Paths like `plan.md` / `description.md` aren't dedicated variables — compose them, e.g. `{workFolder}/changes/inProgress/{xxxx}/plan.md`. Don't substitute anything now; do it at each anchor, at execution time. Nothing else is substituted — a step needing e.g. the current branch runs its own command for it.
...
**Hook: `10-before-analysis`.** Before anything else in this step, if `10-before-analysis.md` (step 1.2) defines `### Step` blocks, run them **now**, in order. `{workFolder}` and `{xxxx}` are substituted. Run each step's command(s) from the repo root and verify what it says it produces. If a step's command fails or its expected output doesn't appear, **stop and explain it to the user** instead of improvising an alternative. If there are no steps, continue silently. (This hook feeds the analysis, so it also runs on a re-analysis; it does **not** run when step 2 chose "implement the current `plan.md`" and jumped to step 3.1.)
...
2. If there are `design_*.html` files under `{changesDir}/inProgress/{xxxx}/mockups/` (visual proposal generated by `pv-new`/`pv-fix`), invoke the configured mockups skill's `action=describe` for the elements you need visual reference for, and build your understanding from that plain-text description — never `Read` a `design_*.html` yourself. Treat that description **only as visual reference** — take from it only the look it illustrates (layout, styles, iconography) for the elements it covers. The technical solution **must not be based on it** in any other sense: don't reuse or literally translate any HTML/CSS/SVG, classes, or markup structure `describe` might mention, nor take it as a reference for architecture, components to create/reuse, or any other implementation decision — you decide all of that from the project's real code (step 5 in this section), exactly as if those files didn't exist.
3. If there are `data_*.md` files loose in `{changesDir}/inProgress/{xxxx}/` (functional data definition generated by `pv-new`/`pv-fix`), treat them as **the source of truth for what data is needed** (which properties or fields, not how they're represented in code today): they're the mandatory starting point for deciding the real technical structure (types, where it's stored, how it's manipulated) when designing the solution — a decision that is indeed yours to make, unlike `design_*.html`'s visual look.
...
**Mandatory step, don't skip or postpone it.** Immediately after writing `plan.md` in the previous step, invoke the `pv-internal-tech-risks` skill (Skill tool) in this same turn, passing it the entry's `plan.md` and `description.md` — it's only invoked at this point, never before the technical solution is decided, because that's when there's enough information to assess risk. It returns the list of the 9 factors scored (0-10) and the final median.

Now write that median to the entry's `.metadata.json` — don't edit `plan.md` and don't write the JSON by hand. Run [`set-metadata.py`](../pv-internal-workflow/scripts/set-metadata.py) from the repo root:

```
python .claude/skills/pv-internal-workflow/scripts/set-metadata.py --xxxx {xxxx} --state inProgress --set-risk {median}
```

(add `--work-folder {workFolder}` if the project configures one; the script creates `.metadata.json` if it doesn't exist yet and leaves `flags`/`flagsLastModified` untouched). The `{median}` is an integer 0-10. The textual "Meaning" ("Moderate risk", etc.) is **not** persisted — it's derivable from the median via `PLAN.template.md`'s table whenever it's needed (e.g. if the user asks in chat).

**Check before continuing:** don't consider `plan.md` finished, nor move to step 3.2, without having confirmed `.metadata.json`'s `risk` holds a real integer 0-10 (re-read the file, or pass `--print` to the command above). Never a placeholder or a missing value.
...
## 3.1.5 Hook: `20-after-plan`

With `plan.md` written and the risk median persisted and verified in `.metadata.json` (step 3.1), and before asking whether to implement (step 3.2), if `20-after-plan.md` (step 1.2) defines `### Step` blocks, run them **now**, in order. `{workFolder}` and `{xxxx}` are substituted; a step that needs `plan.md`'s path composes it as `{workFolder}/changes/inProgress/{xxxx}/plan.md`, and can read the risk median from `.metadata.json` the same way. Run each step's command(s) from the repo root and verify its output. If a step fails, **stop and explain it** — don't improvise an alternative. No steps: continue silently.
