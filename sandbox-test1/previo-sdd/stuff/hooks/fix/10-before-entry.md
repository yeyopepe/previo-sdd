...
No `### Step` blocks below = hook skipped silently. If any step's command fails or its expected output doesn't appear, `pv-fix` stops and explains — it doesn't apply the change and doesn't work around it.
