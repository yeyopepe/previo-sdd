...
No `### Step` blocks below = hook skipped silently. If any step's command fails or its expected output doesn't appear, `pv-new` stops and explains — it doesn't work around it (the entry is already documented on disk; only the external registration is missing).
